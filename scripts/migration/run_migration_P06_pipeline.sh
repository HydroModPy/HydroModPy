#!/usr/bin/env bash
# run_migration_P06_pipeline.sh — Phase P06 : Pipeline 15 steps + checkpoint + BatchRunner
# Usage : ./run_migration_P06_pipeline.sh   (tmux recommandé)
# Estimation : 40h Claude Code

set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/_lib.sh"

PHASE_NAME="P06_pipeline"
BRANCH="migration/P06-pipeline"
...
log "  Branche : $BRANCH"
log "  Spec    : architecture_cible/14_plan_migration.md §11"
log "  Estim   : 40h"
log "================================================================"
...
BRANCHE COURANTE : migration/P06-pipeline (déjà créée)
PLAN DE MIGRATION : architecture_cible/14_plan_migration.md — section §11

Lis la section §11 du plan de migration (objectif, fichiers, tests, critère de
...
2. /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev/architecture_cible/14_plan_migration.md (section §11)
3. Les documents sources référencés dans la section §11 du plan
   (typiquement : un document dans architecture_cible/ + un dans audit_code/).

CONTRAINTES GÉNÉRALES :
- Écrire TOUT en français technique.
- Commits atomiques avec messages "[P06] <action>" en anglais.
...
- Rapporter à la fin : LOC ajoutées, LOC supprimées, tests ajoutés, tests
  cassés (doit être 0), preuves de succès selon critère de la section §11.

CRITÈRE DE SUCCÈS : voir architecture_cible/14_plan_migration.md §11 (sous-section
