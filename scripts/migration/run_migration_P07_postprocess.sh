#!/usr/bin/env bash
# run_migration_P07_postprocess.sh — Phase P07 : Post-traitement : derived + metrics + display UGRID
# Usage : ./run_migration_P07_postprocess.sh   (tmux recommandé)
...
PHASE_NAME="P07_postprocess"
BRANCH="migration/P07-postprocess"
...
log "  Branche : $BRANCH"
log "  Spec    : architecture_cible/14_plan_migration.md §12"
log "  Estim   : 36h"
log "================================================================"
...
BRANCHE COURANTE : migration/P07-postprocess (déjà créée)
PLAN DE MIGRATION : architecture_cible/14_plan_migration.md — section §12

Lis la section §12 du plan de migration (objectif, fichiers, tests, critère de
...
2. /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev/architecture_cible/14_plan_migration.md (section §12)
3. Les documents sources référencés dans la section §12 du plan
   (typiquement : un document dans architecture_cible/ + un dans audit_code/).

CONTRAINTES GÉNÉRALES :
- Écrire TOUT en français technique.
- Commits atomiques avec messages "[P07] <action>" en anglais.
...
- Rapporter à la fin : LOC ajoutées, LOC supprimées, tests ajoutés, tests
  cassés (doit être 0), preuves de succès selon critère de la section §12.

CRITÈRE DE SUCCÈS : voir architecture_cible/14_plan_migration.md §12 (sous-section
