#!/usr/bin/env bash
# run_migration_P03_config.sh — Phase P03 : Config Pydantic v2 : HydroModelBase, UiMeta, Profile, PartialModel
# Usage : ./run_migration_P03_config.sh   (tmux recommandé)
# Estimation : 32h Claude Code

set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/_lib.sh"

PHASE_NAME="P03_config"
BRANCH="migration/P03-config"
...
log "  Branche : $BRANCH"
log "  Spec    : architecture_cible/14_plan_migration.md §8"
log "  Estim   : 32h"
log "================================================================"
...
Tu exécutes la phase P03 du plan de migration HydroModPy.

PROJET : /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev
BRANCHE COURANTE : migration/P03-config (déjà créée)
PLAN DE MIGRATION : architecture_cible/14_plan_migration.md — section §8

Lis la section §8 du plan de migration (objectif, fichiers, tests, critère de
...
2. /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev/architecture_cible/14_plan_migration.md (section §8)
3. Les documents sources référencés dans la section §8 du plan
   (typiquement : un document dans architecture_cible/ + un dans audit_code/).

CONTRAINTES GÉNÉRALES :
- Écrire TOUT en français technique.
- Commits atomiques avec messages "[P03] <action>" en anglais.
...
- Rapporter à la fin : LOC ajoutées, LOC supprimées, tests ajoutés, tests
  cassés (doit être 0), preuves de succès selon critère de la section §8.

CRITÈRE DE SUCCÈS : voir architecture_cible/14_plan_migration.md §8 (sous-section
