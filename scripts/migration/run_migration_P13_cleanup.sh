#!/usr/bin/env bash
# run_migration_P13_cleanup.sh — Phase P13 : Nettoyage : suppression legacy, process/→physics/, docs
# Usage : ./run_migration_P13_cleanup.sh   (tmux recommandé)
# Estimation : 16h Claude Code

set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/_lib.sh"

PHASE_NAME="P13_cleanup"
BRANCH="migration/P13-cleanup"
...
log "  Branche : $BRANCH"
log "  Spec    : architecture_cible/14_plan_migration.md §18"
log "  Estim   : 16h"
log "================================================================"
...
Tu exécutes la phase P13 du plan de migration HydroModPy.

PROJET : /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev
BRANCHE COURANTE : migration/P13-cleanup (déjà créée)
PLAN DE MIGRATION : architecture_cible/14_plan_migration.md — section §18

Lis la section §18 du plan de migration (objectif, fichiers, tests, critère de
...
2. /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev/architecture_cible/14_plan_migration.md (section §18)
3. Les documents sources référencés dans la section §18 du plan
   (typiquement : un document dans architecture_cible/ + un dans audit_code/).

CONTRAINTES GÉNÉRALES :
- Écrire TOUT en français technique.
- Commits atomiques avec messages "[P13] <action>" en anglais.
...
- Rapporter à la fin : LOC ajoutées, LOC supprimées, tests ajoutés, tests
  cassés (doit être 0), preuves de succès selon critère de la section §18.

CRITÈRE DE SUCCÈS : voir architecture_cible/14_plan_migration.md §18 (sous-section
