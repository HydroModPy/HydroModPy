#!/usr/bin/env bash
# run_migration_P09_cli.sh — Phase P09 : CLI : _cli/ éclaté, argcomplete, wizard, messages Levenshtein
# Usage : ./run_migration_P09_cli.sh   (tmux recommandé)
...
log "  Branche : $BRANCH"
log "  Spec    : architecture_cible/14_plan_migration.md §14"
log "  Estim   : 24h"
log "================================================================"
...
Tu exécutes la phase P09 du plan de migration HydroModPy.

PROJET : /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev
BRANCHE COURANTE : migration/P09-cli (déjà créée)
PLAN DE MIGRATION : architecture_cible/14_plan_migration.md — section §14

Lis la section §14 du plan de migration (objectif, fichiers, tests, critère de
...
2. /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev/architecture_cible/14_plan_migration.md (section §14)
3. Les documents sources référencés dans la section §14 du plan
   (typiquement : un document dans architecture_cible/ + un dans audit_code/).

CONTRAINTES GÉNÉRALES :
- Écrire TOUT en français technique.
- Commits atomiques avec messages "[P09] <action>" en anglais.
...
- Rapporter à la fin : LOC ajoutées, LOC supprimées, tests ajoutés, tests
  cassés (doit être 0), preuves de succès selon critère de la section §14.

CRITÈRE DE SUCCÈS : voir architecture_cible/14_plan_migration.md §14 (sous-section
