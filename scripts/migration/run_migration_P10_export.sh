#!/usr/bin/env bash
# run_migration_P10_export.sh — Phase P10 : Export ALL : arborescence structurée + CF-1.11 + GeoPackage
# Usage : ./run_migration_P10_export.sh   (tmux recommandé)
# Estimation : 20h Claude Code

set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/_lib.sh"

PHASE_NAME="P10_export"
BRANCH="migration/P10-export"
...
log "  Branche : $BRANCH"
log "  Spec    : architecture_cible/14_plan_migration.md §15"
log "  Estim   : 20h"
log "================================================================"
...
BRANCHE COURANTE : migration/P10-export (déjà créée)
PLAN DE MIGRATION : architecture_cible/14_plan_migration.md — section §15

Lis la section §15 du plan de migration (objectif, fichiers, tests, critère de
...
2. /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev/architecture_cible/14_plan_migration.md (section §15)
3. Les documents sources référencés dans la section §15 du plan
   (typiquement : un document dans architecture_cible/ + un dans audit_code/).

CONTRAINTES GÉNÉRALES :
- Écrire TOUT en français technique.
- Commits atomiques avec messages "[P10] <action>" en anglais.
...
- Rapporter à la fin : LOC ajoutées, LOC supprimées, tests ajoutés, tests
  cassés (doit être 0), preuves de succès selon critère de la section §15.

CRITÈRE DE SUCCÈS : voir architecture_cible/14_plan_migration.md §15 (sous-section
