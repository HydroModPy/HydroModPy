#!/usr/bin/env bash
# run_migration_P02_storage.sh — Phase P02 : Storage DuckDB 16 tables + Zarr v3 UGRID + migrations + .hmp
# Usage : ./run_migration_P02_storage.sh   (tmux recommandé)
# Estimation : 36h Claude Code

set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/_lib.sh"

PHASE_NAME="P02_storage"
BRANCH="migration/P02-storage"
...
log "  Branche : $BRANCH"
log "  Spec    : architecture_cible/14_plan_migration.md §7"
log "  Estim   : 36h"
log "================================================================"
...
Tu exécutes la phase P02 du plan de migration HydroModPy.

PROJET : /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev
BRANCHE COURANTE : migration/P02-storage (déjà créée)
PLAN DE MIGRATION : architecture_cible/14_plan_migration.md — section §7

Lis la section §7 du plan de migration (objectif, fichiers, tests, critère de
...
2. /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev/architecture_cible/14_plan_migration.md (section §7)
3. Les documents sources référencés dans la section §7 du plan
   (typiquement : un document dans architecture_cible/ + un dans audit_code/).

CONTRAINTES GÉNÉRALES :
- Écrire TOUT en français technique.
- Commits atomiques avec messages "[P02] <action>" en anglais.
...
- Rapporter à la fin : LOC ajoutées, LOC supprimées, tests ajoutés, tests
  cassés (doit être 0), preuves de succès selon critère de la section §7.

CRITÈRE DE SUCCÈS : voir architecture_cible/14_plan_migration.md §7 (sous-section
