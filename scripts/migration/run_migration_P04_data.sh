#!/usr/bin/env bash
# run_migration_P04_data.sh — Phase P04 : Data input : HTTPClient, InputCatalog, sources, hmp data, lockfile
# Usage : ./run_migration_P04_data.sh   (tmux recommandé)
# Estimation : 60h Claude Code
...
log "  Branche : $BRANCH"
log "  Spec    : architecture_cible/14_plan_migration.md §9"
log "  Estim   : 60h"
log "================================================================"
...
BRANCHE COURANTE : migration/P04-data (déjà créée)
PLAN DE MIGRATION : architecture_cible/14_plan_migration.md — section §9

Lis la section §9 du plan de migration (objectif, fichiers, tests, critère de
...
2. /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev/architecture_cible/14_plan_migration.md (section §9)
3. Les documents sources référencés dans la section §9 du plan
   (typiquement : un document dans architecture_cible/ + un dans audit_code/).

CONTRAINTES GÉNÉRALES :
- Écrire TOUT en français technique.
- Commits atomiques avec messages "[P04] <action>" en anglais.
...
- Rapporter à la fin : LOC ajoutées, LOC supprimées, tests ajoutés, tests
  cassés (doit être 0), preuves de succès selon critère de la section §9.

CRITÈRE DE SUCCÈS : voir architecture_cible/14_plan_migration.md §9 (sous-section
