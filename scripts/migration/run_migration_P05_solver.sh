#!/usr/bin/env bash
# run_migration_P05_solver.sh — Phase P05 : Solveurs : SolverPlugin/Runner/Extractor, registry unifié, 3 plugins
# Usage : ./run_migration_P05_solver.sh   (tmux recommandé)
# Estimation : 48h Claude Code

set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/_lib.sh"

PHASE_NAME="P05_solver"
BRANCH="migration/P05-solver"
...
log "  Branche : $BRANCH"
log "  Spec    : architecture_cible/14_plan_migration.md §10"
log "  Estim   : 48h"
log "================================================================"
...
Tu exécutes la phase P05 du plan de migration HydroModPy.

PROJET : /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev
BRANCHE COURANTE : migration/P05-solver (déjà créée)
PLAN DE MIGRATION : architecture_cible/14_plan_migration.md — section §10

Lis la section §10 du plan de migration (objectif, fichiers, tests, critère de
...
2. /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev/architecture_cible/14_plan_migration.md (section §10)
3. Les documents sources référencés dans la section §10 du plan
   (typiquement : un document dans architecture_cible/ + un dans audit_code/).

CONTRAINTES GÉNÉRALES :
- Écrire TOUT en français technique.
- Commits atomiques avec messages "[P05] <action>" en anglais.
...
- Rapporter à la fin : LOC ajoutées, LOC supprimées, tests ajoutés, tests
  cassés (doit être 0), preuves de succès selon critère de la section §10.

CRITÈRE DE SUCCÈS : voir architecture_cible/14_plan_migration.md §10 (sous-section
