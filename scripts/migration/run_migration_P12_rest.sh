#!/usr/bin/env bash
# run_migration_P12_rest.sh — Phase P12 : API REST : hydromodpy/api/ FastAPI + Arrow + WS/SSE
# Usage : ./run_migration_P12_rest.sh   (tmux recommandé)
...
log "  Branche : $BRANCH"
log "  Spec    : architecture_cible/14_plan_migration.md §17"
log "  Estim   : 40h"
log "================================================================"
...
BRANCHE COURANTE : migration/P12-rest (déjà créée)
PLAN DE MIGRATION : architecture_cible/14_plan_migration.md — section §17

Lis la section §17 du plan de migration (objectif, fichiers, tests, critère de
...
2. /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev/architecture_cible/14_plan_migration.md (section §17)
3. Les documents sources référencés dans la section §17 du plan
   (typiquement : un document dans architecture_cible/ + un dans audit_code/).

CONTRAINTES GÉNÉRALES :
- Écrire TOUT en français technique.
- Commits atomiques avec messages "[P12] <action>" en anglais.
...
- Rapporter à la fin : LOC ajoutées, LOC supprimées, tests ajoutés, tests
  cassés (doit être 0), preuves de succès selon critère de la section §17.

CRITÈRE DE SUCCÈS : voir architecture_cible/14_plan_migration.md §17 (sous-section
