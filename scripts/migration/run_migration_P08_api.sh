#!/usr/bin/env bash
# run_migration_P08_api.sh — Phase P08 : API Python : hmp.* top-level + Simulation + SimulationCatalog
# Usage : ./run_migration_P08_api.sh   (tmux recommandé)
# Estimation : 28h Claude Code
...
log "  Branche : $BRANCH"
log "  Spec    : architecture_cible/14_plan_migration.md §13"
log "  Estim   : 28h"
log "================================================================"
...
BRANCHE COURANTE : migration/P08-api (déjà créée)
PLAN DE MIGRATION : architecture_cible/14_plan_migration.md — section §13

Lis la section §13 du plan de migration (objectif, fichiers, tests, critère de
...
2. /home/bb/Documents/01_Git_Repository/02-HydroModPy-dev/architecture_cible/14_plan_migration.md (section §13)
3. Les documents sources référencés dans la section §13 du plan
   (typiquement : un document dans architecture_cible/ + un dans audit_code/).

CONTRAINTES GÉNÉRALES :
- Écrire TOUT en français technique.
- Commits atomiques avec messages "[P08] <action>" en anglais.
...
- Rapporter à la fin : LOC ajoutées, LOC supprimées, tests ajoutés, tests
  cassés (doit être 0), preuves de succès selon critère de la section §13.

CRITÈRE DE SUCCÈS : voir architecture_cible/14_plan_migration.md §13 (sous-section
