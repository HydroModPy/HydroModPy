...
def main() -> None:
    from hydromodpy.validity_frame.storage.ingest_jsonl_to_duckdb import (
        ingest_jsonl_directory,
        ingest_jsonl_file,
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to target duckdb file (will be created if missing)",
    )
    parser.add_argument(
        "--pattern", type=str, default="*.jsonl", help="Glob pattern when using --dir"
    )
    ...
    try:
        if args.jsonl:
            ...
            print(
                f"Ingested {result.rows_ingested}/{result.rows_read} rows, skipped {result.rows_skipped}"
            )
        else:
            ...
            print(
                f"Ingested {total_ingested}/{total_read} rows across {len(results)} files, skipped {total_skipped}"
            )
