    ...
    *,
    ...
    with T.step("hydroprops"):
        ...
        try:
            ...
            df["ksat1"] = _meanfill(ksat, zero_to_mean=True) / 864.0
...
#%% Stats
def param_grid_stats(workdir):
    df = pd.read_csv(Path(workdir) / "input_grid.csv")
    position = list(range(4, 11)) + list(range(12, 15)) + list(range(16, 21)) + [22]
    index = [p - 1 for p in position]
    current = df.iloc[:, index]

    out = pd.DataFrame(
        [current.mean().round(4),
         np.sqrt(current.var()).round(4),
         current.min(),
         current.max()],
        index=["mean", "std", "min", "max"]
    )

    csv_out = Path(workdir) / "pyhelp_preprocessing_stats.csv"
    mode = "a" if Path(csv_out).exists() else "w"
    with open(csv_out, mode, encoding="utf-8", newline="") as f:
        f.write("### section param\n")
        f.write(out.to_csv(index=True))
