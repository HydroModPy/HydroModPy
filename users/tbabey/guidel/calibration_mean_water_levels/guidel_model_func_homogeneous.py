...
def func_objguidel(x,args):
    ...
    print('nRMSE for K='+str(K)+'m/s ='+str(nrmse)+'.')
...
# res = np.zeros((len(logK),len(logvka)))

# for i in list(range(len(K))):
#     for j in list(range(len(vka))):
#         res[i,j] = func_objguidel((logK[i],logvka[j]),BV)
        
    
#%% PLOT

fpath = 'C:/Users/trist/Documents/research/Guidel/calibration/results_homogeneous_anisotrope_500m.csv'
res = pd.read_csv(fpath, sep = '\t', index_col ='K_ms')

logKmin = np.log10(np.min(res.index.values))
logKmax = np.log10(np.max(res.index.values))

vkamin = np.min(res.columns.values.astype(float))
vkamax = np.max(res.columns.values.astype(float))

# 2D Heatmap nRMSE homogeneous anisotropic model
plt.imshow(res.iloc[:,:], 
            interpolation='none',
            aspect = 'auto',
            extent=[vkamin-0.5,vkamax+0.5,logKmax,logKmin])
plt.xlabel("Kh / Kv [-]")
plt.ylabel("log(Kh) [m/s]")
plt.title("Homogeneous anisotropic model")
plt.colorbar(label='nRMSE [-]')
plt.savefig('C:/Users/trist/Documents/research/Guidel/calibration/results_homogeneous_anisotrope.jpg',
            bbox_inches='tight')
plt.show()
...
plt.xscale('log')
plt.ylabel("nRMSE [-]")
plt.legend(['Kh/Kv = 1', 'Kh/Kv = 10'])
plt.title("Homogeneous anisotropic model")
plt.savefig('C:/Users/trist/Documents/research/Guidel/calibration/results_homo_ani-1-10.jpg',
            bbox_inches='tight')
plt.show()

#%% Model results vs data for best case scenarios:simulations
bestK_ani1 = res.index.values[res.iloc[:,0]==np.min(res.iloc[:,0])][0]
res1, wellref, comparedf, shrenv = func_objguidel_full((np.log10(bestK_ani1),np.log10(1)),BV)

# bestK_ani1 = 1e-6
# res10, wellref, comparedf, shrenv = func_objguidel_full((np.log10(bestK_ani1),np.log10(1)),BV)

bestK_ani10 = res.index.values[res.iloc[:,-1]==np.min(res.iloc[:,-1])][0]
res10, wellref, comparedf, shrenv = func_objguidel_full((np.log10(bestK_ani10),np.log10(10)),BV)
...
plt.plot(wellref.index,res10[wellref.index].iloc[0,:],'b')
plt.ylabel('Water table elevation [masl]')
plt.title('Homogeneous anisotropic model')
plt.legend(['Kh/Kv = 1', 'Kh/Kv = 10', 'Observations'])
plt.savefig('C:/Users/trist/Documents/research/Guidel/calibration/results_homo_ani-piezos.jpg',
            bbox_inches='tight')
plt.show()
