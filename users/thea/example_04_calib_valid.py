...
study_site = 'LA_FLUME'
calib_name = 'calib3_RA'
first_year = 1981
last_year = 2024
first_year_calib = 1981
last_year_calib = 2000
first_year_valid = 2001
last_year_valid = 2024
freq_input = 'M'
sim_state = 'transient' 
start_parameters = "1e-6_1%"
...
watershed_name = '_'.join([
    study_site,calib_name,start_parameters,str(first_year),str(last_year),freq_input,sim_state
])
...
# Reanalyse
# # Ensure all variables in var_list exist in varnames_dict
# valid_vars = ['recharge', 'runoff', 'precip', 'evt', 'etp', 't', 'eff_rain']
# BV.climatic.update_sim2_reanalysis(var_list=valid_vars,
...
Qobsmm_calib = select_period(Qobsmm, first_year_calib, last_year_calib)
Qobsmm_valid = select_period(Qobsmm, first_year_valid, last_year_valid)
...
R_calib = select_period(R, first_year_calib, last_year_calib)
r_calib = select_period(r, first_year_calib, last_year_calib)
R_valid = select_period(R, first_year_valid, last_year_valid)
r_valid = select_period(r, first_year_valid, last_year_valid)
...
ax.set_xlim(pd.to_datetime(f'{first_year}'), pd.to_datetime(f'{last_year}'))
ax.set_xlabel('Date')
...
if run_optimization:
    ...
    calib_start_date = f"{first_year_calib}-01-01"
    calib_end_date = f"{last_year_calib}-12-31"
...
if run_optimization:
    ...
    def erreur_modele_norm(params_norm):
        ...
        Qmod_calib= Smod['outflow_drain'] 
        Qmod_calib = Qmod_calib.squeeze()
        Qmod_calib = Qmod_calib*1000 #convert to mm
        if freq_input == 'M':
            Qmod_calib = (Qmod_calib + (r_calib * 1000)) * Qmod_calib.index.day
        elif freq_input == 'W':
            Qmod_calib = (Qmod_calib + (r_calib * 1000)) * 7
        print (f'valeur de Qmod : {Qmod_calib}')

        Qobs_stat = select_period(Qobsmm_calib , first_year_calib, last_year_calib)

        Qmod_stat = select_period(Qmod_calib,first_year_calib,last_year_calib)
    ...
    # Update the model name with the best parameters
    model_name = f"final_optimized_sy{best_sy*100:.4f}_hk{best_hk/24/3600:.2e}%" #! hk{best_hk/24/3600:.2e} rajouter le plot de hk decay soi on en calibre une en plus 
    BV.settings.update_model_name(model_name)
    BV.settings.update_check_model(plot_cross=False, check_grid=True)
    ...
    Qobs_stat_calib = select_period(Qobsmm_calib,first_year_calib,last_year_calib)

    Qmod_stat_calib = select_period(Qmod_calib,first_year_calib,last_year_calib)

    Qmod_stat_Qobs_stat = {
        'Qmod_stat': Qmod_stat_calib,
        'Qobs_stat': Qobs_stat_calib
        }
            
    import hydroeval as he
    NSE_calib = he.evaluator(he.nse, Qmod_stat_calib, Qobs_stat_calib)[0]
    NSElog_calib = he.evaluator(he.nse, Qmod_stat_calib, Qobs_stat_calib, transform='log')[0]
    RMSE_calib = np.sqrt(np.nanmean((Qobs_stat_calib.values-Qmod_stat_calib.values)**2))
    KGE_calib = he.evaluator(he.kge, Qmod_stat_calib, Qobs_stat_calib)[0][0]
    print(model_name.upper())
    print(f" NSE_calib {NSE_calib}")
    print(f" NSElog_calib {NSElog_calib}")
    print(f" RMSE_calib {RMSE_calib}")
    print(f" KGE_calib {KGE_calib}")

    # Store metrics in DataFrame
    metrics_df = pd.DataFrame({
        'model_name': [model_name],
        'NSE': [round(NSE_calib, 2)],
        'NSElog': [round(NSElog_calib, 2)],
        'RMSE': [round(RMSE_calib, 2)],
        'KGE': [round(KGE_calib, 2)]
        })
    ...
    Qmod_df = pd.DataFrame(Qmod_Qobs)
    Qmod_df.to_csv(os.path.join(optim_folder,'Qmod_Qobs_results.csv'))
    Qmod_stats_df = pd.DataFrame(Qmod_stat_Qobs_stat)
    Qmod_stats_df.to_csv(os.path.join(optim_folder,'Qmod_stat_Qobs_stat_results.csv'))
...
# #%% RUN BEST MODELE FOR CALIBRATION PERIOD
# BV.settings.update_model_name(model_name)
# BV.settings.update_check_model(plot_cross=False, check_grid=True)
...
#%% UPDATE
model_name_valid = f"valid_sy{best_sy*100:.4f}_hk{best_hk/24/3600:.2e}%"
# Import modules
BV.add_settings()
BV.add_climatic()
BV.add_hydraulic()
...
# Frame settings
BV.settings.update_model_name(model_name_valid)
BV.settings.update_box_model(box)
BV.settings.update_sink_fill(sink_fill)
BV.settings.update_simulation_state(sim_state)
BV.settings.update_check_model(plot_cross=plot_cross)
...
BV.hydraulic.update_hk(best_hk)
# BV.hydraulic.update_hk_decay(hk_decay) # 0.003
BV.hydraulic.update_sy(best_sy) # 0.1/100
BV.hydraulic.update_cond_drain(cond_drain)
...
#%% RUN BEST MODELE FOR VALIDATION PERIOD
BV.settings.update_model_name(model_name)
BV.settings.update_check_model(plot_cross=False, check_grid=True)
...
Qmod_valid = Smod['outflow_drain'] 
Qmod_valid = Qmod_valid.squeeze()
Qmod_valid = Qmod_valid*1000

if freq_input == 'W' :
    Qobsmm_valid = (Qmod_valid + (r_valid * 1000)) * 7
if freq_input == 'M':
    Qmod_valid = (Qmod_valid + (r_valid * 1000)) * Qmod_valid.index.day
    
print (f'valeur de Qmod : {Qmod_valid}')
# Rmod = Smod['recharge'] 
...
ax = a0
ax.plot(Qobsmm_valid, color='k', lw=1, ls='-', zorder=0, label='observed')
ax.plot(Qmod_valid, color='red', lw=1, label='modeled')
...
ax.set_xlim(pd.to_datetime(f'{first_year_valid}-01'), pd.to_datetime(f'{last_year_valid}-12'))
...
Qobs_stat_valid = select_period(Qobsmm_valid,first_year_valid,last_year_valid)
    
Qmod_stat_valid = select_period(Qmod_valid,first_year_valid,last_year_valid)
        
import hydroeval as he
NSE_valid = he.evaluator(he.nse, Qmod_stat_valid, Qobs_stat_valid)[0]
NSElog_valid = he.evaluator(he.nse, Qmod_stat_valid, Qobs_stat_valid, transform='log')[0]
RMSE_valid = np.sqrt(np.nanmean((Qobs_stat_valid.values-Qmod_stat_valid.values)**2))
KGE_valid  = he.evaluator(he.kge, Qmod_stat_valid, Qobs_stat_valid)[0][0]
print(model_name.upper())
print(f" NSE {NSE_valid}")
print(f" NSElog {NSElog_valid}")
print(f" RMSE {RMSE_valid}")
print(f" KGE {KGE_valid}")

# Store metrics in DataFrame
metrics_df = pd.DataFrame({
    'model_name': [model_name],
    'NSE': [round(NSE_valid, 2)],
    'NSElog': [round(NSElog_valid, 2)],
    'RMSE': [round(RMSE_valid, 2)],
    'KGE': [round(KGE_valid, 2)]
})
...
ax = a1
ax.scatter(Qobs_stat_valid, Qmod_stat_valid,
...
#%% PLOT DISCHARGE FOR CALIB AND VALID PERIODS

fig, (a0, a1) = plt.subplots(1, 2, gridspec_kw={'width_ratios': [3, 1]},
                                figsize=(10,3))
ax = a0
ax.plot(Qobsmm, color='k', lw=1, ls='-', zorder=0, label='observed')
ax.plot(Qmod_calib, color='red', lw=1, label='modeled (calibrated)')
ax.plot(Qmod_valid, color='red', lw=1, label='modeled (validated)', ls ='--')
# ax.plot(Qnormalized*1000, label='Qnormalized', c='orange', lw=0.5)
...
metrics_df = pd.DataFrame({
    'model_name': [model_name],
    'NSE_calib': [round(NSE_calib, 2)],
    'NSElog_calib': [round(NSElog_calib, 2)],
    'RMSE_calib': [round(RMSE_calib, 2)],
    'KGE_calib': [round(KGE_calib, 2)]
})
...
KGE_valid = he.evaluator(he.kge, Qmod_stat_valid, Qobs_stat_valid)[0][0]
print(model_name.upper())
print(f" NSE_valid {NSE_valid}")
print(f" NSElog_valid {NSElog_valid}")
print(f" RMSE_valid {RMSE_valid}")
print(f" KGE_valid {KGE_valid}")

# Store metrics in DataFrame
metrics_df = pd.DataFrame({
    'model_name': [model_name],
    'NSE_valid': [round(NSE_valid, 2)],
    'NSElog_valid': [round(NSElog_valid, 2)],
    'RMSE_valid': [round(RMSE_valid, 2)],
    'KGE_valid': [round(KGE_valid, 2)]
})
...
ax = a1
ax.scatter(Qobs_stat_calib, Qmod_stat_calib,
            s=10, edgecolor='none', alpha=0.90, color='forestgreen', marker='+')  # Circle marker
ax.scatter(Qobs_stat_valid, Qmod_stat_valid,
            s=10, edgecolor='none', alpha=0.90, color='deepskyblue', marker='+')  # Triangle marker
ax.set_xscale('log')
ax.set_yscale('log')
ax.plot((0.01,1000),(0.01,1000), color='grey', zorder=-1)
ax.set_xlim(0.01,1000)
