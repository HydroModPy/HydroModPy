...
def analyze_calibration_grid(folder_path: str, 
    ...
    if isinstance(color_column, dict):
        ...
        normalize = color_column.get('normalize', False)
    ...
    if color_column in df_clean.columns:
        ...
        if len(z_data) > 0:
            norm, norm_description = determine_color_normalization(
                z_data.values, color_column, color_log, vmin, vmax
            )
...
def explore_parameter_grid(folder_path: str) -> Optional[pd.DataFrame]:
    ...
    param_keywords = ['hk','exdp', 'sy', 'k', 'conductivity', 'porosity', 'storage', 'thick']
...
def quick_combined_plot(folder_path: str,
    ...
    Examples
    --------
    >>> # Quick plot combining NSElog and FA with auto color range
    >>> fig, ax, df = quick_combined_plot(path, ['NSElog', 'FA'])
    
    >>> # With custom weights and fixed color range 0 to 1
    >>> fig, ax, df = quick_combined_plot(path, ['NSElog', 'FA'], weights=[1, 1], vmin=0, vmax=1)
    """
...
def quick_grid_plot(folder_path: str, 
    ...
    >>> fig, ax, df = quick_grid_plot(path, 'exdp', 'sy', 'NSE')
    
    >>> # With fixed color range from 0 to 1
    >>> fig, ax, df = quick_grid_plot(path, 'exdp', 'sy', 'NSE', vmin=0, vmax=1)
    """
...
if __name__ == "__main__":
    folder_path = r"C:\Users\theat\Documents\Python\01_Git_Repository\01-HMPdev-sewage\users\thea\parameters"
    if os.path.exists(folder_path):
        print_metric_definitions()
        
        df = explore_parameter_grid(folder_path)
        if df is not None:
            # Example 1: Standard plot with auto color range
            fig1, ax1, df1 = quick_grid_plot(folder_path, 'exdp', 'sy', 'NSElog', vmin=0, vmax=1)
            
            # Example 2: Combined metric with custom color range 0 to 1
            ...
            # )
            #
            # Example 3: NSE with fixed range -1 to 1 (theoretical NSE range)
            # fig3, ax3, df3 = quick_grid_plot(folder_path, 'exdp', 'sy', 'NSE', vmin=-1, vmax=1)
