...
def run_single_simulation(hk,exdp, sy, base_config, iteration_name):
    """
    ...
    try:
        ...
        logging.info(f"Worker {worker_id}: Starting simulation hk={hk}, exdp={exdp}, sy={sy}")
        ...
        BV.hydraulic.update_hk(config['hk'] * 3600 * 24)  # Fixed parameter from config
        ...
        logging.error(f"Simulation failed for hk={hk}, exdp={exdp}, sy={sy}: {str(e)}")
        return {
            'success': False,
            'hk': hk,
            'exdp': exdp,
...
def worker_function(worker_id, parameters_file, base_config):
    ...
    try:
        ...
        for idx, row in df_params.iterrows():
            ...
            results = run_single_simulation(hk, exdp, sy, worker_config, iteration_name)
            end_time = time.time()
...
def main():
    ...
    base_config = {
        ...
        'streamflow_obs_file': 'J721401001.csv',  # streamflow observations file
        
        # Fixed hydraulic parameters
        'hk': 1.88e-5,  # Fixed hydraulic conductivity (m/s) - ADJUST THIS VALUE AS NEEDED
        'thick': 30,
        'nlay': 1,
        'lay_decay': 1,
        'bottom': None,
