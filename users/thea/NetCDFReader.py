...
class NetCDFReader:
    ...
    def md(self):
        ...
        meta = {}
        try:
            meta['bbox'] = self.ds.rio.bounds()
        except:
            meta['bbox'] = None
        try:
            meta['start_date'] = pd.to_datetime(str(self.ds.time[0].values)).strftime("%Y-%m-%d")
        except:
            meta['start_date'] = None
        try:
            meta['end_date'] = pd.to_datetime(str(self.ds.time[-1].values)).strftime("%Y-%m-%d")
        except:
            meta['end_date'] = None
        try:
            meta['timestep'] = pd.to_timedelta((self.ds.time[1] - self.ds.time[0]).values).isoformat()
        except:
            meta['timestep'] = None
        try:
            meta['crs'] = self.ds.rio.crs.to_epsg()
        except:
            meta['crs'] = None
        try:
            meta['shape'] = self.ds.rio.shape
        except:
            meta['shape'] = None
        try:
            meta['resolution'] = self.ds.rio.resolution()
        except:
            meta['resolution'] = None
        try:
            meta['variables'] = list(self.ds.data_vars)
        except:
            meta['variables'] = None
        try:
            meta['global_attributes'] = {attr: self.ds.attrs[attr] for attr in self.ds.attrs}
        except:
            meta['global_attributes'] = None
        try:
            meta['dimensions'] = {dim: len(self.ds[dim]) for dim in self.ds.dims}
        except:
            meta['dimensions'] = None
        try:
            meta['variable_attributes'] = {var: self.ds[var].attrs for var in self.ds.data_vars}
        except:
            meta['variable_attributes'] = None
        return meta
...
#%% Extract time series for a specific variable at a given point
    extraction_point = [344966, 6797471, 2154]
    variable = "DRAINC_Q"
...
#%% Plot
    reader.plot_interactive(variable="ETP_Q",
