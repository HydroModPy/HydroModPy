...
class Visualization():
    ...
    def visual3D(self, object_list=['grid', 'watertable'] ,
        ...
        for i in range (0,len(object_list)):
            ...
            # print(obj)
            if obj == 'grid':
                plt.show(grid_mesh,contour,"Topographic elevation [m]", at=i,
                         camera=cam, viewup='z', axes = 13, bg=bg)
            if obj == 'watertable':
                plt.show(grid_wireframe,contour, watertable_elev,"Watertable elevation [m]",
                         camera=cam, viewup ='z', at=i, axes = 13, bg=bg)
            if obj == 'watertable_depth':
                plt.show(grid_wireframe,contour, watertable_depth,"Watertable depth [m]",
                         camera=cam, viewup ='z', at=i, axes = 13, bg=bg)
            if obj == 'surface_flow':
            ...
            if obj == 'drain_flow':
                ...
                plt.show(grid_wireframe,contour, watertable_blue, drain_flow,"Seepage outflow log [m3/d]",
                         camera=cam, viewup ='z', at=i, axes = 13, bg=bg)
