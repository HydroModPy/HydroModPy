...
watershed_name = '_'.join([
    "Example_03",study_site,parameters,str(first_year),str(last_year)
])
...
from_xyv = [344964,6797466, 150, 10 , 'EPSG:2154'] # [x, y, snap distance, buffer size, crs proj]
...
##%%% Reanalyse
#* ces lignes la elles vont chercher toutes les data de sim 2
BV.climatic.update_sim2_reanalysis(var_list=['recharge', 'runoff','precip' #'recharge', 'runoff', 'precip','evt', 'etp', 't'
                                              ],
...
#%% Update simdata #todo ATTENTION PAR RERUN SINON 10-3 S'ACCUMULE
#* ces lignes là elles mettent à jour la recharge et runoff en fonction de sim_state i.e si = steady ca fait la moyenne 
...
########## LOOP ##########
list_hyd_cond = np.geomspace(1e-5,1e-1,5) * 24 * 3600 # m/day #* geomspace = fonction de numpy pour générer 10 valeurs a intervalle égale entre les deux bornes données
...
#%% UPDATE
#* ces lignes là elles mettent à jour les valeurs des paramètres = celles entrées au dessus
# Import modules
BV.add_settings()
BV.add_climatic()
BV.add_hydraulic()
...
BV.settings.update_dis_perlen(dis_perlen=dis_perlen) #* active ou non la discretisation dans le temps de la recharge

# Particle tracking settings
#BV.settings.update_input_particles(zone_partic=BV.geographic.watershed_box_buff_dem) # or 'seepage_path'

#%% MODFLOW
#* va sortir les simulations en fonction des valeurs de k en entrée #paramétrization
...
for hyd_cond in list_hyd_cond:
    ...
    model_name = iD_set_simulations+'_'+str(round(hyd_cond/24/3600,5))
...
#%% CROSS
#* va sortir toutes les wateertable elevation par rapport aux valeurs de k en entrée #paramétrization
#* en fonction de l'axe x et z
compt = 1
...
for model_name, success_modflow, model_modflow in zip(list_model_name,
    ...
    wt_v_fill = ax.fill_between(np.arange(xx.shape[0])*75,
                                ...
                                color='dodgerblue', alpha=0.5, lw=0, label='Watertable')
    # Line watertable
    w_prof = ax.plot(np.arange(xx.shape[0])*75, wt_v_plot, color='navy', lw=1, label='Watertable Line')
    
    # Facecolor unsaturated
    wt_v_fill = ax.fill_between(np.arange(xx.shape[0])*75, 
                                wt_v_plot, dem_v_plot,
                                color='saddlebrown', alpha=0.5, lw=0, label='Unsaturated Zone')
    
    # Line unsaturated
    d_prof = ax.plot(np.arange(xx.shape[0])*75, dem_v_plot, 'saddlebrown', lw=1.5, label='DEM Line')
    
    # Facecolor noflow
    ax.fill_between(np.arange(xx.shape[0])*75,
                    0, dem_v_plot-30,
                    color='lightgrey', alpha=1, lw=0, zorder=10, label='No Flow Zone')
    # Line noflow
    ax.plot(np.arange(xx.shape[0])*75, dem_v_plot-30, color='dimgray', lw=1.5, label='No Flow Line')
    
    # Settings
    ax.set_xlim(1980, 4000)
    ax.set_ylim(85, 130)
    ...
    # Add legend
    ax.legend(loc='upper right') #, bbox_to_anchor=(0.5, -0.05), ncol=3
...
#%% MAP
#* va sortir toutes les seepage par rapport aux valeurs de k en entrée #paramétrization
#* en focntion de l'axe x et y 
#todo vérifier que c'est les seepages
compt = 0
...
for model_name, success_modflow, model_modflow in zip(list_model_name,
    ...
    im_dem = ax.imshow(dem_data, alpha=0.5, cmap='Greys', label='DEM')
    im_cont = ax.imshow(contour, alpha=1, cmap=mpl.colors.ListedColormap(['k']), label='Contour')
    im_obs = ax.imshow(obs_river_data, alpha=1, cmap=mpl.colors.ListedColormap(['navy']), label='Observed River')
    im_sim = ax.imshow(sim_river_data, cmap=mpl.colors.ListedColormap(['red']), alpha=0.7, label='Simulated River')
    im_seep = ax.imshow(seep_river_data, cmap=mpl.colors.ListedColormap(['darkorange']), alpha=0.7, label='Seepage Areas')
...
#%% GRAPH
#* sort la densité de drainage en fonction de la conductivité hydraulique
