...
# Qobs = Qobs.iloc[:, :2]
# Qobs.columns = [None, 'Q']
# Qobs[None] = pd.to_datetime(Qobs[None], format='%Y-%m-%d')
# Qobs.set_index(None, inplace=True)
# Qobs = Qobs.squeeze()

# Qobs = Qobs.iloc[:, :2]
# Qobs.columns = [None, 'Q']
# Qobs[None] = pd.to_datetime(Qobs[None], format='%d/%m/%Y')
# Qobs.set_index(None, inplace=True)
...
if run_optimization:
    ...
    def erreur_modele_norm(params_norm):
        ...
        model_name = f"optim_{compt}_{timestamp}_hk{hk_value/24/3600:.2e}_sy{sy_value*100:.4f}%"
        logging.info(f"\nSimulation {compt}: hk={hk_value/24/3600:.2e}m/s, sy={sy_value*100:.4f}%")
        ...
        errorNSElog = (1-NSElog)**2 # Squared error to match 1
        ...
        obs_pixel = mean_obsf_to_simf/75
        sim_pixel = mean_simf_to_obsf/75
        ...
        error = (errormatchingstream+errorNSElog)/2 # Average error between NSElog and matching streams
        # Save the results    
        current_simulation = {
    ...
    print(f"Valeur initiale hk: {hk_init:.2e} m/jour ({hk_init/24/3600:.2e} m/s), log(hk): {log_hk_init:.4f}, sy: {sy_init:.4f}")
