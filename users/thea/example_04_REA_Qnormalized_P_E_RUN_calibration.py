...
watershed_name = '_'.join(['test_calib_P_E_RUN_2022', study_site])
...
# BV.add_subbasin(os.path.join(data_path, 'additional'), 150, subbasin_name='add_ONDES_FLUME')
# BV.add_subbasin(os.path.join(data_path, 'additional'), 150, subbasin_name='add_LANGAN')
...
# BV.climatic.update_recharge(BV.climatic.recharge, sim_state = sim_state) # from mm to m
...
BV.climatic.update_runoff(BV.climatic.runoff, sim_state=sim_state) # from mm to m
...
#%%RECHARGE REANALYSIS
# BV.climatic.recharge = BV.climatic.precip - BV.climatic.evt - BV.climatic.runoff # recharge = precip - evapotranspiration
# BV.climatic.update_recharge(BV.climatic.recharge, sim_state = sim_state) # from mm to m
# BV.climatic.update_recharge(BV.climatic.recharge.resample('M').sum(), sim_state = sim_state) # days to month
...
ax.plot(q, label='Q', c='orange', lw=1)
ax.set_yscale('log')
...
Qobs_mm_day = select_period(Qobs,2020,2022)  # m/day to mm/day
Qobs_mm_day = Qobs_mm_day * 1000  # m/day to mm/day

fig, ax1 = plt.subplots(figsize=(8, 5))

# Plot Qobs on the left y-axis
ax1.plot(Qobs_mm_day, label='Qobs', c='navy', lw=1)
...
ax2 = ax1.twinx()
ax2.plot(precip_mm_day, label='precip', c='blue', lw=1)
...
ax.plot(Qnormalized, label='Qnormalized', c='orange', lw=0.5)
...
ax.plot(Qobs, label='Qobs', c='green', lw=0.8, linestyle=':')

ax.set_xlabel('Date')
ax.set_yscale('log')
ax.set_ylabel(f'[m/{freq_input}]')
# ax.set_yscale('log')
# ax.set_ylim(-1e-1, 1)
...
#%% -- dataframe to save input information
information = pd.DataFrame({
    ...
    'nlay': nlay,
    'lay_decay': lay_decay,  # 1 for no decay
    'bottom': bottom,        # elevation in meters, None for constant aquifer thickness, or 2D matrix
    'thick': thick,          # if bottom is None, aquifer thickness
    'hk': hk,                # m/day
    'cond_drain': cond_drain, # or value of conductance
    'sy': sy,                # [-]
}, index=[0])

# Save the information DataFrame to a CSV file
information_path = os.path.join(watershed_path, 'model_information.csv')

#%% CLASS MatchingStreams
#* cette classe compare ma localisation géographique de mon réseau simulé et de mon réseau observé.
class MatchingStreams:
    """ 
...
if run_optimization:
    ...
    def erreur_modele_norm(params_norm):
        ...
        current_iteration = compt
        
        # Vérifications de sécurité
        print(f"params_norm: {params_norm}")
        print(f"len(params_norm): {len(params_norm)}")
        
        if len(params_norm) < 2:
            print("Erreur: params_norm n'a pas assez d'éléments")
            return 1e6
        
        # Vérifiez que Qobsmonthmm n'est pas vide
        print(f"Qobsmonthmm shape: {Qobsmonthmm.shape}")
        print(f"Qobsmonthmm index length: {len(Qobsmonthmm.index)}")
        
        if len(Qobsmonthmm.index) == 0:
            print("Erreur: Qobsmonthmm est vide")
            return 1e6
        ...
        model_name = f"optim_{current_iteration}_{timestamp}_hk{hk/24/3600}_sy{sy*100}%_th{thick}"
        ...
        print("Qobsweekmm index:", Qobsweekmm.index)
        print("filtered_dates:", filtered_dates)

        for date in filtered_dates:
            print(f"Processing date: {date}")
            # adapte la variable a entrer en fonction de freq_input
            if date in Qobsmonthmm.index:
                observed_Q.append(Qobsmonthmm.loc[date])         #! changer par Qobsmonthmm si freq_input == 'M'
                print(f"Found date: {date}, Q: {Qobsmonthmm.loc[date]}")
            else:
                closest_date = Qobsmonthmm.index[abs(Qobsmonthmm.index - date).argmin()]
                observed_Q.append(Qobsmonthmm.loc[closest_date])
                print(f"Closest date found: {closest_date}, Q: {Qobsmonthmm.loc[closest_date, 'Q']}")
        ...
        errormatching = sim / obs
        print('INDICATORMATCHINGSRTEAM : ', errormatching)
        errormatchingstream = (1-errormatching)**2
        # Store results in dictionary
        optimization_results["model_name"] = model_name
        optimization_results["model_modflow"] = model_modflow
    
        if np.isnan(errormatchingstream) or obs == 0:
            return np.inf  # Large error if indicator is invalid
        
        error = errormatchingstream
        # Save the results    
        current_simulation = {
    ...
    print(f"Bornes hk: [{log_hk_min:.2e}, {log_hk_max:.2e}] m/jour")
...
#%% PRINT EACH VALUE OF ERRORNSELOG AND ERRORMATCHINGSTREAM
fig2, (b0, b1) = plt.subplots(1, 2, gridspec_kw={'width_ratios': [1, 1]},  
                              figsize=(10, 5))
...
bx.set_xlabel('Step')
bx.set_ylabel('Values of NSElog and matchingstream')
bx.legend()
bx.grid()

# Plot errormatchingstream and error_nselog
bx = b1
bx.plot(step, errormatchingstream, label='errormatchingstream', color='green')
# bx.plot(step, error_nselog, label='error_nselog', color='red')
bx.set_xlabel('Step')
bx.set_ylabel('Values of errormatchingstream and error_nselog')
bx.legend()
bx.grid()

# Adjust layout and save the figure
fig2.tight_layout()
fig2.savefig(os.path.join(calibration_folder,'_figures','error_metrics_plot2.png'), dpi=300)
plt.show()
#%% FORMATTING QobsGAUGED STATION CSV 
model_name = "optim_46_132752_hk0.0006493816315762085_sy0.09085937500000003%_th30"
