...
def initialize_watershed(config: ProjectConfig, data_path: str, out_path: str):
    ...
    if config.general.subbassin:
        hydrometry_path = config.paths.hydrometry_path(data_path)
        BV.add_hydrometry(hydrometry_path, config.paths.hydrometry_filename)

        intermittency_path = config.paths.intermittency_path(data_path)
        BV.add_intermittency(intermittency_path, config.paths.intermittency_filename)
        BV.add_subbasin(sub_snap_dist=200)

    geol_path = config.paths.geology_path(data_path)
    BV.add_geology(
        geol_path,
        types_obs=config.paths.geology_types_obs,
        fields_obs=config.paths.geology_fields_obs,
    )

    hydro_path = config.paths.hydrography_path(data_path)
    BV.add_hydrography(
        hydro_path,
        types_obs=list(config.paths.hydrography_types_obs),
        fields_obs=list(config.paths.hydrography_fields_obs),
    )
...
def configure_reservoir(
    ...
    mask_path = config.reservoir.mask_path(data_path)
    if not os.path.exists(mask_path):
        raise FileNotFoundError(f"Masque réservoir introuvable: {mask_path}")
    ...
    bathy_path = config.reservoir.bathymetry_path(data_path)
    if not os.path.exists(bathy_path):
        raise FileNotFoundError(f"Bathymétrie réservoir introuvable: {bathy_path}")
    BV.lakeres.update_bathymetry(lake_id, bathy_path)

    outlet_path = config.reservoir.outlet_path(data_path)
    if outlet_path:
        if not os.path.exists(outlet_path):
            raise FileNotFoundError(f"Exutoire alternatif introuvable: {outlet_path}")
