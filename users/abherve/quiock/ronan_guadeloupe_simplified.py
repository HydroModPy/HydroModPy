...
fig, ax_l0 = plt.subplots(1,1, figsize=(4,3.1))
# ax_r0 = ax_l0.twinx()
# ax_r0 = ax_l0.twinx()
...
ax_l0.plot(data_explo['k0k1'], data_explo['Doptim'],
            color='red', marker='o', lw=2)
ax_l0.plot(data_explo['k0k1'], data_explo['D_so'],
            color='grey', marker='o', lw=2)
ax_l0.plot(data_explo['k0k1'], data_explo['D_os'],
            color='grey', marker='o', lw=2)
...
# ax_l0.set_ylim(0.001, 1e3)
ax_l0.set_xlabel('K1 / K2')
ax_l0.set_ylabel('Criteria, CC1 and CC2 [-]', c='k')
# ax_l0.set_title('Calibration criteria')
# ax_l0.axvline(x=6e-1, c='k', ls='--')
# ax_l0.axvline(x=2, c='limegreen', ls='--', lw=2)

ax_l0.plot(data_explo['k0k1'], (abs(1-(data_explo['Qout_sim']/data_explo['Qsub_obs']))),
            color='dodgerblue', marker='^', ms=8, lw=2, mew=1.5, mec='dodgerblue', mfc='white')
# ax_l0.set_yscale('log')
# ax_l0.set_ylim(0.1, 30)
# ax_l0.set_ylabel('', c='dodgerblue',  labelpad=+25,)
# ax_l0.set_ylim(0.01, 10)
...
ax_r1.plot(data_explo['k0k1'], 
           data_explo['RMSE']/(0.7092-0.7041),
           color='darkorange',
           marker='o', ms=8, lw=2, mew=1.5, mec='darkorange', mfc='white')
ax_r1.set_ylim(0,1)

ax_r1.set_ylabel('$RMSE_{norm}$, CC3 [-]', c='k',
                 rotation=270, labelpad=+35,) #'\u0394$GW_{storage}$ $[Mm^3$]', $^{87}$Sr/$^{86}$Sr'
"""
# ax_l0.set_xscale('log')
# ax_l0.set_xlim(0.001,1000)
...
ax.plot(data_explo['k0k1'], (data_explo['k0']/3600/24*40 + data_explo['k1']/3600/24*360)/400,
        color='grey', marker='|', lw=1.5, mec='grey', mfc='w', ms=10, mew=1.5, label='Keq')
ax.plot(data_explo['k0k1'], data_explo['k0']/3600/24,
        color='forestgreen', marker='o', lw=1.5, mec='forestgreen', mfc='w', mew=1.5, label='K1')
ax.plot(data_explo['k0k1'], data_explo['k1']/3600/24,
        color='darkviolet', marker='o', lw=1.5, mec='darkviolet', mfc='w', mew=1.5, label='K2')
...
fig.savefig(fig_path+'K1etK2'+'allcases'+'.png', dpi=300, bbox_inches='tight')
...
ax.plot(data_explo['k0k1'], data_explo['D_so'],
            color='grey', marker='o', lw=1.5, mec='grey', mfc='w', mew=1.5)
ax.plot(data_explo['k0k1'], data_explo['D_os'],
            color='k', marker='o', lw=1.5, mec='k', mfc='w', mew=1.5)
# ax_l0.plot(data_explo['k0k1'], (abs(data_explo['D_so']-data_explo['D_os'])),
            # color='darkred', marker='s', lw=2, mew=1.5, mec='darkred', mfc='white')
# ax_l0.set_yscale('log')
ax.plot(data_explo['k0k1'], data_explo['Doptim'],
...
# ax.set_ylim(-3, 100)
ax.set_xlabel('K1 / K2 [-]')
ax.set_ylabel('Distance criteria [m]', c='k')

# ax.grid()

fig.savefig(fig_path+'obj streams_'+'allcases'+'.png', dpi=300, bbox_inches='tight')

#%% A') MAPS STREAMS

streams = gpd.read_file("C:/Users/ronan/Documents/SIMULATIONS/GUADELOUPE/Quiock2/results_stable/hydrology/L_Quiock_creek2.shp")
contour = gpd.read_file("C:/Users/ronan/Documents/SIMULATIONS/GUADELOUPE/Quiock2/results_stable/geographic/watershed_contour.shp")

import rasterio
from osgeo import gdal
from rasterio.plot import show

dem = rasterio.open("C:/Users/ronan/Documents/SIMULATIONS/GUADELOUPE/Quiock2/results_stable/geographic/watershed_dem.tif")
...
for model_name in list_selects[:]:
    ...
    fig.savefig(fig_path+'map streams_'+model_name+'.png', dpi=300, bbox_inches='tight')
...
ax.plot(data_explo['k0k1'], data_explo['Qout_sim']/data_explo['Qsub_obs'],
...
ax.set_ylim(1, None)
ax.set_xlabel('K1 / K2 [-]')
ax.set_ylabel('$Q_{sim}$ / $Q_{obs}$ [-]', c='k')

# ax.grid()

fig.savefig(fig_path+'obj streamflow_'+'allcases'+'.png', dpi=300, bbox_inches='tight')
...
fig.savefig(fig_path+'obj sr_'+'allcases'+'.png', dpi=300, bbox_inches='tight')
...
for model_name in list_selects[-2:-1]:
    ...
    sub = imageio.imread('C:/Users/ronan/Documents/SIMULATIONS/GUADELOUPE/Quiock3/results_stable/subbasin/subbasin_Flowrate/watershed_dem.tif')
    sub_mask = np.ma.masked_where(dem.read(1) < -100, sub)
    sub_mask = sub_mask.filled(np.nan)
    sub_mask[sub_mask<=0] = np.nan
    plt.imshow(sub_mask)

    sub = imageio.imread('C:/Users/ronan/Documents/SIMULATIONS/GUADELOUPE/Quiock3/results_stable/hydrology/L_Quiock_creek2.tif')
    sub_mask = np.ma.masked_where(dem.read(1) < -100, sub)
    sub_mask = sub_mask.filled(np.nan)
    sub_mask[sub_mask<=0] = np.nan
    plt.imshow(sub_mask)
    ...
    plt.plot(flat_acc[1], flat_acc[0])
    plt.plot(flat_acc[1], flat_acc[0])
    ...
    plt.plot(flat_cum_acc.index*5, flat_cum_acc[0])
    flat_cum_acc[3] = flat_cum_acc[0].cumsum() # 3 is cumulated on sum xaxis in 1
    plt.plot(flat_cum_acc.index*5, flat_cum_acc[3])
    ...
    flat_cum_acc = flat_cum_acc.reset_index()
    
    for i in flat_cum_acc.index:
        if i <36:
            flat_cum_acc.loc[i,'a'] = (flat_cum_acc.loc[i, 1] - 100) * (230/40) - 470
        else:
            flat_cum_acc.loc[i,'a'] = (flat_cum_acc.loc[i, 1] - 152) * (470/52)
    ...
    ax.step(flat_cum_acc['a'], flat_cum_acc[3], c='k', lw=2, marker='.', ms=0, zorder=10)
    # ax.step(flat_cum_shal.index, flat_cum_shal[3], c='b', lw=2, marker='.', ms=0, zorder=10)
    # ax.step(flat_cum_deep.index, flat_cum_deep[3], c='r', lw=2, marker='.', ms=0, zorder=10)
...
for model_name in list_selects[-2:-1]:
    ...
    # flat_flux = flat_flux.reindex(index=flat_flux.index[::-1])
    # flat_flux = flat_flux.set_index(flat_flux.index[::-1])
    
    flat_flux.index = flat_flux.index * 5
    flat_flux.index = abs(flat_flux.index-flat_flux.index.max())
    ...
    d = field['d_mouth'].astype(int)
    
    def round_multiple(numb, mul):
        return mul * round(numb/mul)
    
    for iv in d.index:
        d.loc[iv] = round_multiple(d.loc[iv], 5)
    ...
    d.loc[7,'d_mouth'] = 5
    d.loc[12,'d_mouth'] = 10
    d.loc[13,'d_mouth'] = 125
    d.loc[15,'d_mouth'] = 475
    d.loc[18,'d_mouth'] = 480
    ...
    for i in flat_flux.index:
        ...
        for j in d['d_mouth']:
            if i == j:
                # d.loc[j,'Rsr_riv']['Rsr_riv'] = flat_flux.loc[i,'Rsr_riv']
                # list_sim.append(flat_flux.loc[i,'Rsr_riv'])
                d.loc[d[d['d_mouth']==j].index,'Rsr_riv'] = flat_flux.loc[i,'Rsr_riv']
            else:
                d.loc[d[d['d_mouth']==j].index,'Rsr_riv'] = np.nan
            cp+=1
    
    diff = d.copy()
    
    # value = np.sqrt(np.nansum((np.array(diff['Rsr_riv'])-np.array(diff['o']))**2)/len(o))
    sim_a = np.array(flat_flux['Rsr_riv'])
    obs_a = np.array(flat_flux['o'])
    
    sim_int = sim_a[~np.isnan(obs_a)]
    
    sim_mask = sim_int.copy()
    sim_mask[sim_int==0] = np.nan
    
    obs_mask = obs_a[~np.isnan(obs_a)]
    obs_mask[sim_int==0] = np.nan
    
    the_diff = sim_mask-obs_mask
    value=np.sqrt(np.nansum((the_diff)**2)/len(o))
    print(value)
    data_explo.loc[cpt,'RMSE'] = value
    
    fig, ax = plt.subplots(1,1, figsize=(4,4))
    ax.plot(flat_flux['o'], flat_flux['Rsr_riv'], marker='o', c='forestgreen', lw = 0, ms=13)
...
#%% CORRIG CONCENTRATION SR - ALON RIVERS
...
for model_name in list_selects[-2:-1]:
    ...
    flat_cum_acc.index = flat_cum_acc[1]
    
    flat_flux['dist_riv'] = flat_cum_acc['a']
    flat_flux['Qriv'] = flat_cum_acc[3]
    flat_flux['Qdeep'] = flat_cum_deep[0]
    flat_flux['Qshal'] = flat_cum_shal[0]
    first_index = flat_flux['Qriv'].first_valid_index()
    ...
    flat_flux['Csr_riv_calc'] = ''
    flat_flux['Rsr_riv_calc'] = ''
    flat_flux['Qriv_local_deepshall'] = '' #ce calcul suit le bilan de masse : somme shallow and deep

    for i in flat_flux.index:
        if i == 0:
            flat_flux.loc[i,'Qriv_deepshall'] = flat_flux.loc[i,'Qdeep']+flat_flux.loc[i,'Qshal']
        else :
            flat_flux.loc[i,'Qriv_deepshall'] = flat_flux.loc[i-1,'Qriv_deepshall']+flat_flux.loc[i,'Qdeep']+flat_flux.loc[i,'Qshal']

        
    for i in flat_flux.index:
        if i == 0:
            flat_flux.loc[i,'Csr_riv_calc'] = Srp
            flat_flux.loc[i,'Rsr_riv_calc'] = RSrp
        else:
            flat_flux.loc[i,'Csr_riv_calc'] = ((flat_flux.loc[i-1,'Csr_riv_calc']*flat_flux.loc[i-1,'Qriv_deepshall'])+\
                                                (Srr*flat_flux.loc[i,'Qdeep'])+(Srp*flat_flux.loc[i,'Qshal']) ) / (flat_flux.loc[i,'Qriv_deepshall'])
                                             
            flat_flux.loc[i,'Rsr_riv_calc'] = ((flat_flux.loc[i-1,'Rsr_riv_calc']*flat_flux.loc[i-1,'Csr_riv_calc']*flat_flux.loc[i-1,'Qriv_deepshall'])+\
                                               (RSrr*Srr*flat_flux.loc[i,'Qdeep'])+ (RSrp*Srp*flat_flux.loc[i,'Qshal']) ) / ((flat_flux.loc[i,'Qriv_deepshall']*flat_flux.loc[i,'Csr_riv_calc']))
    ...
    flat_flux.index = abs(flat_flux.index-flat_flux.index.max())
    #%%
    ax.plot(flat_flux['dist_riv'], flat_flux['Csr_riv_calc'], c='tomato', marker='o', lw=0, mec='darkorange', mew=1, mfc='none')
    axb.plot(flat_flux['dist_riv'], flat_flux['Rsr_riv_calc'], c='grey', marker='o', lw=0, mec='darkviolet', mew=1, mfc='none')
    # ax.step(flat_flux.index, flat_flux['Csr_riv'], color='tomato',
    #         # marker=None, markeredgecolor='none',
    #         # markersize=5, lw=1, label='upstream',
    #         # where=step
    ...
    for i in flat_flux.index:
        ...
        for j in d['d_mouth']:
            if i == j:
                # d.loc[j,'Rsr_riv']['Rsr_riv'] = flat_flux.loc[i,'Rsr_riv']
                # list_sim.append(flat_flux.loc[i,'Rsr_riv'])
                d.loc[d[d['d_mouth']==j].index,'Rsr_riv_calc'] = flat_flux.loc[i,'Rsr_riv_calc']
            else:
                d.loc[d[d['d_mouth']==j].index,'Rsr_riv_calc'] = np.nan
            cp+=1
    
    diff = d.copy()
    
    # value = np.sqrt(np.nansum((np.array(diff['Rsr_riv'])-np.array(diff['o']))**2)/len(o))
    sim_a = np.array(flat_flux['Rsr_riv_calc'])
    obs_a = np.array(flat_flux['o'])
    ...
    #%%
    fig, ax = plt.subplots(1,1, figsize=(4,4))
    ax.plot(flat_flux['o'], flat_flux['Rsr_riv_calc'], marker='o', c='forestgreen', lw = 0, ms=13)
    ax.set_xlim(0.704, 0.710)
    ax.set_ylim(0.704, 0.710)
    ax.ticklabel_format(style='plain')
    ax.ticklabel_format(useOffset=False, style='plain')
...
stre = gpd.read_file('C:/Users/ronan/Documents/SIMULATIONS/GUADELOUPE/Quiock2/results_stable/hydrology/L_Quiock_creek2.shp')
...
for model_name in list_selects[:]:
    ...
    shp_ending_shal = gpd.read_file(simulations_folder+
                              model_name+'/'+'_pathlines/'+
                              'shp_ending_shal.shp') # time in years !
    shp_ending_shal = shp_ending_shal[shp_ending_shal.time > 0]
    shp_ending_shal = shp_ending_shal[shp_ending_shal.i0.astype(str)+'-'+shp_ending_shal.j0.astype(str) !=
                                      shp_ending_shal.i.astype(str)+'-'+shp_ending_shal.j.astype(str)]
    shp_ending_shal = shp_ending_shal[shp_ending_shal.k <= 1]
    
    shp_ending_deep = gpd.read_file(simulations_folder+
                              model_name+'/'+'_pathlines/'+
                              'shp_ending_deep.shp') # time in years !
    shp_ending_deep = shp_ending_deep[shp_ending_deep.time > 0]
    ...
    stre.plot(ax=ax, color='navy', lw=1.5, zorder=-1)
    ...
    stre.plot(ax=ax, color='navy', lw=1.5, zorder=-1)
    shp_box.plot(ax=ax, facecolor='none', lw=2, zorder=10)
    ep = shp_ending.plot(ax=ax, column='time', cmap='RdYlGn_r', lw=0, markersize=4,
                              norm=mpl.colors.LogNorm(vmin=1, vmax=10))
    shp_pathlines_spings.plot(ax=ax, column='time', cmap='RdYlGn_r', lw=0.5,
    ...
    sm = plt.cm.ScalarMappable(cmap='RdYlGn_r', norm=mpl.colors.LogNorm(vmin=1, vmax=100))
