...
#%% Extract site data tools   
    ...
    def do_site_mask(self, site_id,
        ...
        if os.path.exists(site_mask_path) and not reset:
            vprint(f'>> Load existing mask file {site_mask_path}')
            mask = np.load(site_mask_path)
            vprint('>>< END > do_site_mask')
            if checkplot:
        ...
        # II. if not exists - generate it
            ...
            vprint(f'>> Generate new mask for site {site_id}')
            # 1. Load site box
            vprint(f'>> load site box from {site_box_path}')
            ...
            if checkplot:
                Geo.plot_multiple_gdfs([gdf_site], labels = ['site box'],
                                    save = f'{output_path}checkplot/{site_id}_catchement_box.pdf')
            # 2. Identify corner pixel in the grid
            vprint(f'>> Identify data site box pixel corners')   
...
#%% Timeserie statistics
    ...
    def compute_timeserie_statistics(self, variable, shape_path = False, 
        ...
        for t in range(len(timeline)):
            timestep = np.array(self.dataset[variable][t,:,:].values)
