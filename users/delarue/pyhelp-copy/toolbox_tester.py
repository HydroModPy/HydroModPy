...
if wsTest: 
    path_ws = 'Z:/_waterwise_teams_database/_save/_20250319/_time_series/_testing_sites/_sado/_climate/_air_temperature/_observation/_MALGA SADOlE/'    
    ws = tb.WeatherStation(path_ws, variables = ['2m_temperature'])
    print(ws._infos)
    print(ws._gdf)
    print(ws._data)
...
if testTimeserieStats:
    ...
    stats = data.compute_timeserie_statistics(shape_path, f'{output_path}test_urse.csv', var = '2m_temperature')
    
    # plot result
    fig, ax = plt.subplots(1,1,figsize = [10,5])

    stats.plot(ax = ax, x = 'time')
    ax.set_xlim(['2013-01-01','2014-01-01'])
    ax.set_ylim([-35,30])
