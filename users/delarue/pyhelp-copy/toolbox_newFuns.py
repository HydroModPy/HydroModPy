...
class HelperCERRA():
    ...
    def generate_gridFile(ref_path, grid_path, attrs = dict(), coord_ids = dict(), src_crs = 6258, verbose = False):     
...
#%% Extract site data tools   
    ...
    # TODO ddocumentation
    def generate_site_mask(self, site_id, type_site, database_folder, output_folder, buffer, catch_crs = 3035,
                          obs_variables = ['air_temperature'], verbose = False, checkplot = False, save = True):   
        
        vprint = print if verbose else lambda *a, **k: None
        if checkplot:
            create_folder(f'{output_folder}checkplot/')
        ...
        obs_paths = [f'{obs_path}_{v}/_observation/{station_id}/' for v in obs_variables for station_id in os.listdir(f'{obs_path}_{v}/_observation/')]
        list_points = []
        for path in obs_paths:
            try:
                ws = WeatherStation(path)
                list_points.append(ws._gdf.geometry[0])
            except:
        ...
        if checkplot:
            Geo.plot_multiple_gdfs([gdf_catch,gdf_obs], labels = ['site','observation'],
                                   save = f'{output_folder}checkplot/{site_id}_catch_obs.pdf')
        ...
        if checkplot:            
            ...
            Geo.plot_multiple_gdfs([gdf_grid,gdf_mask], labels=['grid','mask'],
                                   ...
                                   markersize = [5],
                                   save = f'{output_folder}checkplot/{site_id}_mask.pdf')
          
        # IV. save mask
        np.save(f'{output_folder}/{site_id}_mask.npy', mask)            
        return mask
        ...
        for var in variables:            
            ...
            for year in years:
                ...
                if os.path.isfile(input_file_path):                
                    ...
                    vprint(' no data', end = '')
                vprint(' - ', end = '')
            combine_file = f'{output_path}{var}_{site_id}.nc'
...
#%% Cerra data to Help input    
    ...
    def generate_pyHelp_input(self, gdf_helpGrid, df_helpGrid, var, rule = 'nearest', timestep = 'D',
