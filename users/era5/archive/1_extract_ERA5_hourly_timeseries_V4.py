# -*- coding: utf-8 -*-
"""
Created on Sun Jan 26 07:45:39 2025
...
for site in sites:
    ...
    for variable in variables:
        ...
        # Process the first year and month to determine buffer size
        if not buffer_determined:
            first_year = sorted(os.listdir(variable_path))[0]
            year_path = os.path.join(variable_path, first_year)
            first_month_file = sorted(os.listdir(year_path))[0]

            if first_month_file.endswith('.nc'):
                file_path = os.path.join(year_path, first_month_file)
        ...
        if not buffer_determined:
            print(f"Skipping site {site} due to failed buffer determination.")
            break
        ...
        for year in sorted(os.listdir(variable_path)):
            ...
            for month_file in sorted(os.listdir(year_path)):
                ...
                if month_file.endswith('.nc'):
                    ...
                    # Clip the data using the determined buffer size
                    buffered_polygon = polygon.buffer(buffer_size)
                    clipped_data = dataset.rio.clip(buffered_polygon.geometry, buffered_polygon.crs, drop=True)
