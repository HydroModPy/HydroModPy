...
folder_path_t2sm = './t2m'
name_t2sm = 't2m_hourly.csv'
file_t2sm = os.path.join(folder_path_t2sm, name_t2sm)
hourly_df_t2sm = pd.read_csv(file_t2sm, index_col=0)
hourly_df_t2sm.index = pd.to_datetime(hourly_df_t2sm.index)

yearly_t2sm = hourly_df_t2sm.resample('Y').mean()
...
folder_path_tp = './tp'
name_tp = 'tp_hourly.csv'
file_tp = os.path.join(folder_path_tp, name_tp)
hourly_df_tp = pd.read_csv(file_tp, index_col=0)
hourly_df_tp.index = pd.to_datetime(hourly_df_tp.index)

yearly_tp = hourly_df_tp.resample('Y').mean()
...
mean_ref_t2sm = hourly_df_t2sm.loc[(hourly_df_t2sm.index >= start_ref) & (hourly_df_t2sm.index < end_ref), 't2m'].mean()
hourly_df_t2sm['anomaly'] = hourly_df_t2sm['t2m'] - mean_ref_t2sm
...
mean_ref_tp = hourly_df_tp.loc[(hourly_df_tp.index >= start_ref) & (hourly_df_tp.index < end_ref), 'tp'].mean()
hourly_df_tp['anomaly'] = (hourly_df_tp['tp'] - mean_ref_tp)/mean_ref_tp*100
...
plt.savefig('./figures/anomaly_tp_t2sm.svg', format='svg')
