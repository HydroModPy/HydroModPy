import os
import pandas as pd
import matplotlib.pyplot as plt

# Load your hourly temperature data 
# Paths and settings
output_folder = r'\\vert\CHYN_OBSERVATOIRE_POSCHIAVINO\_Alps\_waterwise_process\_climate\_era5'
sites = ["_cont", "_jamt", "_gdsa", "_rech", "_sado", "_zugs", "_peca"]

for site in sites:
    print('####################################################################')
    print(f"Processing site: {site}")
    print('')
    site_folder = os.path.join(output_folder, site)
    fig_folder = os.path.join(site_folder, 'fig')
    os.makedirs(fig_folder, exist_ok=True)

    variable = ['2m_temperature']
      
    name = variable[0] + '.csv'
    file = os.path.join(site_folder, name)
    
    hourly_df = pd.read_csv(file, index_col=0)
    hourly_df.index = pd.to_datetime(hourly_df.index)
    
    # Reorder the dataframe chronologically by sorting the index
    hourly_df = hourly_df.sort_index()
    
    #%%
    
    # Calculate the anomalies relative to a reference period (e.g., 2000-2010)
    start_ref = '1980-01-01'
    end_ref = '2000-01-01'
    mean_ref = hourly_df.loc[(hourly_df.index >= start_ref) & (hourly_df.index < end_ref), 'mean'].mean()
    hourly_df['anomaly'] = hourly_df['mean'] - mean_ref
    
    # Group the data by month to create monthly anomalies
    monthly_anomalies = hourly_df.resample('M').mean()
    
    # Calculate the rolling 12-month average
    rolling_12m_avg = monthly_anomalies['anomaly'].rolling(window=12, min_periods=1).mean()
    
    # Plot the anomalies with custom styling
    fig, axes = plt.subplots(2, 1, figsize=(12, 8), sharex=True)
    
    # Set background color
    fig.patch.set_facecolor('white')
    
    #%% Plot monthly anomalies
    axes[0].bar(monthly_anomalies.index, monthly_anomalies['anomaly'], 
                color=['blue' if anomaly <= 0 else 'red' for anomaly in monthly_anomalies['anomaly']],
                width = 50)
    
    #axes[0].grid(axis='y', linestyle='--', linewidth=0.5, color='gray')
    axes[0].set_ylabel('Anomaly [°C]', fontsize=14, labelpad=15)
    axes[0].set_title('Monthly Surface Air Temperature Anomalies [°C]', fontsize=12, pad=20)
    axes[0].set_ylim(monthly_anomalies['anomaly'].min() - 0.5, monthly_anomalies['anomaly'].max() + 0.5)
    axes[0].set_yticks(range(int(axes[0].get_ylim()[0]), int(axes[0].get_ylim()[1]) + 1, 2))
    axes[0].set_yticklabels(['{:+d}'.format(y) for y in range(int(axes[0].get_ylim()[0]), int(axes[0].get_ylim()[1]) + 1, 2)], fontsize=8)
    
    # Plot rolling 12-month average excluding first and last 12 months
    rolling_12m_avg_trimmed = rolling_12m_avg.iloc[12:-12]
    axes[1].plot(rolling_12m_avg_trimmed.index, rolling_12m_avg_trimmed, color='black')
    axes[1].fill_between(rolling_12m_avg_trimmed.index, rolling_12m_avg_trimmed, 0, where=rolling_12m_avg_trimmed >= 0, facecolor='red', interpolate=True)
    axes[1].fill_between(rolling_12m_avg_trimmed.index, rolling_12m_avg_trimmed, 0, where=rolling_12m_avg_trimmed <= 0, facecolor='blue', interpolate=True)
    axes[1].grid(axis='y', linestyle='--', linewidth=0.5, color='gray')
    axes[1].set_ylabel('Anomaly [°C]', fontsize=14, labelpad=15)
    axes[1].set_title('12-Month Rolling Average Surface Air Temperature Anomalies [°C]', fontsize=12, pad=20)
    axes[1].set_ylim(rolling_12m_avg_trimmed.min() - 0.5, rolling_12m_avg_trimmed.max() + 0.5)
    axes[1].set_yticks(range(int(axes[1].get_ylim()[0]), int(axes[1].get_ylim()[1]) + 1, 1))
    axes[1].set_yticklabels(['{:+d}'.format(y) for y in range(int(axes[1].get_ylim()[0]), int(axes[1].get_ylim()[1]) + 1, 1)], fontsize=8)
    
    # Customize tick marks and labels
    for ax in axes:
        ax.set_xticks(monthly_anomalies.index[::24])  # Display every other year
        ax.set_xticklabels(monthly_anomalies.index.year.unique()[::2], rotation=45, ha='right', fontsize=8)
        # ax.set_yticks(range(int(ax.get_ylim()[0]), int(ax.get_ylim()[1]) + 1, 1))
        # ax.set_yticklabels(['{:+d}'.format(y) for y in range(int(ax.get_ylim()[0]), int(ax.get_ylim()[1]) + 1, 1)], fontsize=8)
    
    # Add horizontal line at 0 for both subplots
    for ax in axes:
        ax.axhline(0, color='black', linewidth=1)
    
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    
    plt.tight_layout()
    plt.show()
    
    name_fig = variable[0] + '_anomaly.png'
    fig_name = os.path.join(fig_folder,name_fig)
    fig.savefig(fig_name)
    print(f"Plot saved")