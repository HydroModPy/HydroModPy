"""Comparison workflow for the steady fixed-head piecewise-K validation case."""
...
from .runtime_boussinesq import run_boussinesq_fixed_head_piecewise_k_case
...
class BoussinesqFixedHeadPiecewiseKComparison:
...
def build_boussinesq_fixed_head_piecewise_k_comparison(
    *,
    result: ValidationRunResult,
    ...
    analytical_profile = expected_boussinesq_fixed_head_piecewise_profile(
        xmin=float(reference_cfg["xmin"]),
        xmax=float(reference_cfg["xmax"]),
        ncol=numerical_profile.size,
        west_head=float(reference_cfg["west_head"]),
    ...
    return BoussinesqFixedHeadPiecewiseKComparison(
        result=result,
...
def run_boussinesq_fixed_head_piecewise_k_comparison(
    *,
    ...
    if normalized_solver == "boussinesq":
        result = run_boussinesq_fixed_head_piecewise_k_case(
    ...
    return build_boussinesq_fixed_head_piecewise_k_comparison(
        result=result,
        metadata=metadata,
