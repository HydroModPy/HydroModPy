"""Plotting helpers for the steady hillslope-drainage linearized case."""
...
from .comparison import LinearizedUnconfinedHillslopeDrainageComparison
...
def plot_linearized_unconfined_hillslope_drainage_comparison(
    comparison: LinearizedUnconfinedHillslopeDrainageComparison,
    *,
    output_png: str | Path,
    show_plot: bool = True,
    dpi: int = 160,
) -> Path:
    """Save one figure comparing numerical and analytical hillslope profiles."""
    ...
    params_line = (
        ...
        f"z_e={float(reference_cfg['topography_base_elevation_m']):.2f} m   "
        f"dz={float(reference_cfg['topography_right_to_left_amplitude_m']):.2f} m   "
        f"C={float(reference_cfg['drainage_conductance_m2_per_s']):.1e} m2/s"
    )
    ...
    ax_profile.fill_between(
        ...
        comparison.topography_profile.min() - 0.2,
        color="0.88",
        alpha=0.7,
        label="Topography",
        zorder=0,
    ...
    ax_profile.plot(
        ...
        color="tab:orange",
        lw=2.4,
        label="Analytical linearized profile",
        zorder=2,
    )
    ax_profile.scatter(
        comparison.x,
    ...
    metrics_line = (
        ...
        f"min clearance={comparison.min_numerical_clearance_m:.4f} m"
    )
    fig.text(
        0.5,
        0.01,
    ...
    fig.suptitle("Linearized Unconfined Hillslope Drainage 1D Validation", fontsize=13)
