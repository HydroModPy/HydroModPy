"""Local `flow/boussinesq` runtime for the uniform-recharge piecewise-K validation case."""
...
def run_boussinesq_uniform_recharge_piecewise_k_case(
    *,
    ...
    """Run the uniform-recharge piecewise-K case through the local `flow/boussinesq` adapter."""
    del timeout
    ...
    ctx = RunContext(
        plan=SimulationPlan(
            name="Boussinesq uniform-recharge validation",
            description="Steady piecewise-K recharge strip",
            runs=(run,),
        ),
        run=run,
        state=state,
