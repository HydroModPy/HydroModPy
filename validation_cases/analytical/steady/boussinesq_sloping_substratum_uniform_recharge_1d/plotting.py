"""Plotting helpers for the sloping-substratum uniform-recharge case."""
...
def plot_boussinesq_sloping_substratum_uniform_recharge_comparison(
    comparison: BoussinesqSlopingSubstratumUniformRechargeComparison,
    *,
    output_png: str | Path,
    show_plot: bool = True,
    dpi: int = 160,
) -> Path:
    """Save one figure comparing the numerical and reference head profiles."""
    ...
    flux_line = (
        f"q_w,ref={comparison.reference_west_discharge_per_width_m2_s:.3e} m2/s   "
        f"q_e,ref={comparison.reference_east_discharge_per_width_m2_s:.3e} m2/s"
    )
    ...
    ax_profile.plot(
        ...
        color="tab:orange",
        lw=2.4,
        label="Semi-analytical Boussinesq profile",
        zorder=2,
    )
    ax_profile.scatter(
        comparison.x,
    ...
    ax_profile.set_title("Numerical and reference water-table profiles")
    ...
    ax_residual.plot(
        ...
        marker="o",
        ms=4.4,
        label="Residual (numerical - reference)",
        zorder=3,
    )
    ...
    fig.suptitle("Boussinesq Sloping-Substratum Uniform-Recharge 1D Validation", fontsize=13)
