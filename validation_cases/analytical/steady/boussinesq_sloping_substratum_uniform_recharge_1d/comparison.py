"""Comparison workflow for the sloping-substratum uniform-recharge case."""
...
class BoussinesqSlopingSubstratumUniformRechargeComparison:
    """All arrays and diagnostics required by the validation case."""
    ...
    max_error: float
    row_spread: float
    reference_west_discharge_per_width_m2_s: float
    reference_east_discharge_per_width_m2_s: float


def build_boussinesq_sloping_substratum_uniform_recharge_comparison(
    *,
    result: ValidationRunResult,
    metadata: dict | None = None,
    tolerances: dict | None = None,
) -> BoussinesqSlopingSubstratumUniformRechargeComparison:
    """Load one completed run and compare it to the semi-analytical reference."""
    ...
    east_discharge = float(west_discharge) + _mm_day_to_m_s(
        float(reference_cfg["recharge_mm_day"])
    ) * (float(reference_cfg["xmax"]) - float(reference_cfg["xmin"]))

    return BoussinesqSlopingSubstratumUniformRechargeComparison(
        result=result,
        metadata=case_metadata,
        ...
        reference_west_discharge_per_width_m2_s=float(west_discharge),
        reference_east_discharge_per_width_m2_s=float(east_discharge),
    )


def run_boussinesq_sloping_substratum_uniform_recharge_comparison(
    *,
    ...
    if normalized_solver == "boussinesq":
        result = run_boussinesq_sloping_substratum_uniform_recharge_case(
    ...
    return build_boussinesq_sloping_substratum_uniform_recharge_comparison(
        result=result,
        metadata=metadata,
...
__all__ = [
    "BoussinesqSlopingSubstratumUniformRechargeComparison",
    "build_boussinesq_sloping_substratum_uniform_recharge_comparison",
    "run_boussinesq_sloping_substratum_uniform_recharge_comparison",
]
