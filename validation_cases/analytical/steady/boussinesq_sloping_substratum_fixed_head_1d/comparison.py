"""Comparison workflow for the sloping-substratum fixed-head case."""
...
from .runtime_boussinesq import run_boussinesq_sloping_substratum_fixed_head_case
...
class BoussinesqSlopingSubstratumFixedHeadComparison:
...
def build_boussinesq_sloping_substratum_fixed_head_comparison(
    *,
    result: ValidationRunResult,
    ...
    analytical_profile = expected_sloping_substratum_fixed_head_profile_at_x(
        x_m=x,
        xmin=float(reference_cfg["xmin"]),
        xmax=float(reference_cfg["xmax"]),
    ...
    reference_discharge = solve_sloping_substratum_fixed_head_discharge_per_width(
        xmin=float(reference_cfg["xmin"]),
        xmax=float(reference_cfg["xmax"]),
        west_head_m=float(reference_cfg["west_head"]),
        east_head_m=float(reference_cfg["east_head"]),
    ...
    return BoussinesqSlopingSubstratumFixedHeadComparison(
        result=result,
        metadata=case_metadata,
...
def run_boussinesq_sloping_substratum_fixed_head_comparison(
    *,
    ...
    if normalized_solver == "boussinesq":
        result = run_boussinesq_sloping_substratum_fixed_head_case(
    ...
    return build_boussinesq_sloping_substratum_fixed_head_comparison(
        result=result,
        metadata=metadata,
