...
4. Generated children are ordinary workflow files:
   `workflow = "simulation"` for mesh, flow, and transport children, or
   `workflow = "comparison"` for comparison children. Mesh-only children use
   `[[simulation.process]]` with `type = "mesh"` and `backend = "catchment"`.
...
New regional campaigns can enter through `workflow = "testbed"` with
...
`workflow = "comparison"` TOML. A catalog-backed variant typically renders
