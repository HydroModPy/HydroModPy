...
def resolve_structured_shape_from_run_folder(
    run_folder: Path,
) -> tuple[int, int] | None:
...
def _structured_bounds_from_config(
    config_path: Path,
) -> tuple[float, float, float, float] | None:
    if rasterio is None:
        return None
...
def _observable_time_role(
    *,
    ...
    """Describe how one observable row should be interpreted in time."""
