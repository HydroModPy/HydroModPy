...
def _parse_metric_limit(
    *,
    ...
    if not variant_id or not observable:
        raise ValueError(
            f"Metric limit in case {case_id!r} must define variant_id and observable."
        )
    limits: dict[str, float] = {}
...
def _check_metric_limit(
    *,
    ...
    if row is None:
        add_error(
            "required metric row is missing: "
            f"{metric_limit.variant_id}::{metric_limit.observable}"
        )
        return
    ...
    for field, upper_bound in metric_limit.limits.items():
        ...
        if value is None:
            add_error(
                f"{metric_limit.variant_id}::{metric_limit.observable}.{field} is missing"
            )
            continue
