...
def build_numerical_closure_tables(
    *,
    ...
    for row in budget_rows:
        ...
        time_key = (
            f"elapsed_seconds:{elapsed:.12g}"
            if elapsed is not None
            else f"time_index:{time_index}"
        )
        item = grouped.setdefault(
            (simulation_id, time_key),
            {
                ...
                "sort_value": (
                    elapsed
                    if elapsed is not None
                    else _as_float(time_index) or float(len(grouped))
                ),
            },
        )
...
def _summary_rows(
    ...
    for simulation_id, group in sorted(grouped.items()):
        ...
        p95_relative = (
            float(np.nanpercentile(relatives, 95.0)) if relatives.size else math.nan
        )
...
def _diagnostic(relative_error: float, max_abs_mm_d: float) -> str:
    ...
    if (relative is not None and relative <= 1.0e-3) or (
        depth is not None and depth <= 1.0e-3
    ):
        return "OK"
    if (relative is not None and relative <= 1.0e-2) or (
        depth is not None and depth <= 1.0e-2
    ):
        return "WARN"
