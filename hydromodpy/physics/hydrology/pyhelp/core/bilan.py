...
def calc_yearly_streamflow(help_output, surf_output):
    ...
    yearly_qflow["qtot_sim"] = (
        zone_yearly_runoff + zone_yearly_subrun1 + zone_yearly_subrun2 + zone_yearly_rechg
    )

    yearly_qflow["qbase_sim"] = zone_yearly_rechg + zone_yearly_subrun1 + zone_yearly_subrun2

    return yearly_qflow


def plot_sim_vs_obs_yearly_streamflow(sim_qflow, obs_qflow, fig_title, figname=None):
    """
    ...
    ax.set_position(
        [left_margin, bot_margin, 1 - left_margin - right_margin, 1 - top_margin - bot_margin]
    )
    ...
    ax.text(
        0.5, 1, fig_title, fontsize=16, ha="center", va="bottom", transform=ax.transAxes + offset
    )
...
def plot_streamflow_scatter(sim_qflow, obs_qflow, fig_title, figname=None):
    ...
    (l2,) = ax.plot(yearly_qflow["qtot_obs"], yearly_qflow["qtot_sim"], ".", color="#3690c0")
    (l3,) = ax.plot(yearly_qflow["qbase_obs"], yearly_qflow["qbase_sim"], ".", color="#990000")
    ...
    rmse_qbase = np.nanmean((yearly_qflow["qbase_sim"] - yearly_qflow["qbase_obs"]) ** 2) ** 0.5
    me_qbase = np.nanmean(yearly_qflow["qbase_sim"] - yearly_qflow["qbase_obs"])
