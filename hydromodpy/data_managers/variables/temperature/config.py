...
class TemperatureSourceConfig(BaseModel):
...
class TemperatureConfig(BaseModel):
    ...
    sources: Annotated[list[TemperatureSourceConfig], ParamLevel("user")] = Field(
    ...
    @model_validator(mode="after")
    def _check_date_order(self) -> "TemperatureConfig":
    ...
    @classmethod
    def from_toml(cls, path: str | Path) -> "TemperatureConfig":
        ...
        with open(path, "rb") as f:
            data = tomllib.load(f)
        section = data.get("temperature", data)
        cfg = cls.model_validate(section)
        _resolve_paths(cfg, path.parent)
        return cfg


def _resolve_paths(cfg: "TemperatureConfig", toml_dir: Path) -> None:
