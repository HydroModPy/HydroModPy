...
class RadiationSourceConfig(BaseModel):
...
class RadiationConfig(BaseModel):
    ...
    sources: Annotated[list[RadiationSourceConfig], ParamLevel("user")] = Field(..., min_length=1, description="At least one data source.")
    ...
    @model_validator(mode="after")
    def _check_date_order(self) -> "RadiationConfig":
    ...
    @classmethod
    def from_toml(cls, path: str | Path) -> "RadiationConfig":
        ...
        with open(path, "rb") as f:
            data = tomllib.load(f)
        section = data.get("radiation", data)
        cfg = cls.model_validate(section)
        _resolve_paths(cfg, path.parent)
        return cfg


def _resolve_paths(cfg: "RadiationConfig", toml_dir: Path) -> None:
