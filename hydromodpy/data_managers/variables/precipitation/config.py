...
class PrecipitationSourceConfig(BaseModel):
...
class PrecipitationConfig(BaseModel):
    """Top-level precipitation configuration."""

    model_config = ConfigDict(extra="forbid")

    sources: Annotated[list[PrecipitationSourceConfig], ParamLevel("user")] = Field(..., min_length=1, description="At least one data source.")
    ...
    @model_validator(mode="after")
    def _check_date_order(self) -> "PrecipitationConfig":
    ...
    @classmethod
    def from_toml(cls, path: str | Path) -> "PrecipitationConfig":
        ...
        with open(path, "rb") as f:
            data = tomllib.load(f)
        section = data.get("precipitation", data)
        cfg = cls.model_validate(section)
        _resolve_paths(cfg, path.parent)
        return cfg


def _resolve_paths(cfg: "PrecipitationConfig", toml_dir: Path) -> None:
