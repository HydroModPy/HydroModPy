...
class IntermittencyConfig(BaseModel):
    ...
    date_start: Annotated[Optional[str], ParamLevel("user")] = Field(
        default=None, description="Project start date (ISO format, e.g. '2012-01-01')."
    )
    ...
    @model_validator(mode="after")
    def _check_date_order(self) -> "IntermittencyConfig":
    ...
    @classmethod
    def from_toml(cls, path: str | Path) -> "IntermittencyConfig":
        ...
        with open(path, "rb") as f:
            data = tomllib.load(f)
        section = data.get("intermittency", data)
        cfg = cls.model_validate(section)
        _resolve_paths(cfg, path.parent)
        return cfg


def _resolve_paths(cfg: IntermittencyConfig, toml_dir: Path) -> None:
