"""Custom data loader for oceanic (CSV directory, NetCDF, or GeoTIFF)."""
...
def load_custom(
    ...
    *,
    ...
    if path.is_dir():
        return _load_csv(config, project_period=project_period)
    elif path.suffix == ".nc":
...
def _load_csv(
    ...
    *,
    ...
    """Load oceanic records from user CSV files."""
...
def _find_location_file(data_dir: Path) -> Path:
    for ext in ("csv", "shp", "gpkg", "geojson"):
        candidate = data_dir / f"oceanic_custom_LOC.{ext}"
        if candidate.exists():
            return candidate
    raise FileNotFoundError(f"No oceanic_custom_LOC.* in {data_dir}")


def _find_chronicle_file(data_dir: Path, station_id: str) -> Path | None:
    safe_id = safe_file_token(station_id)
    for pattern in (f"oceanic_custom_{safe_id}_*.csv", f"oceanic_custom_{station_id}_*.csv"):
        matches = sorted(data_dir.glob(pattern))
        if matches:
            return matches[0]
    exact = data_dir / f"oceanic_custom_{safe_id}.csv"
