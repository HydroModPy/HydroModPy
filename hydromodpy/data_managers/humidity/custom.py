"""Custom data loader for humidity (user-provided CSV files)."""
...
def load_custom(
    ...
    *,
    ...
    """Load humidity records from user CSV files."""
    ...
    for loc in locations:
        ...
        records.append(
            PointRecord(
                station_id=loc.id, variable="humidity", source="custom",
...
def _find_location_file(data_dir: Path) -> Path:
    for ext in ("csv", "shp", "gpkg", "geojson"):
        candidate = data_dir / f"humidity_custom_LOC.{ext}"
        if candidate.exists():
            return candidate
    raise FileNotFoundError(f"No humidity_custom_LOC.* in {data_dir}")


def _find_chronicle_file(data_dir: Path, station_id: str) -> Path | None:
    safe_id = safe_file_token(station_id)
    for pattern in (f"humidity_custom_{safe_id}_*.csv", f"humidity_custom_{station_id}_*.csv"):
        matches = sorted(data_dir.glob(pattern))
        if matches:
            return matches[0]
    exact = data_dir / f"humidity_custom_{safe_id}.csv"
