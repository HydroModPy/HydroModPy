"""Custom data loader for temperature (user-provided CSV files)."""
...
from hydromodpy.data_managers.temperature.config import TemperatureSourceConfig
...
def load_custom(
    ...
    *,
    ...
    internal_unit: str = "degC",
) -> list[PointRecord]:
    """Load temperature records from user CSV files."""
    ...
    for loc in locations:
        ...
        records.append(
            PointRecord(
                station_id=loc.id, variable="temperature", source="custom",
...
def _find_location_file(data_dir: Path) -> Path:
    for ext in ("csv", "shp", "gpkg", "geojson"):
        candidate = data_dir / f"temperature_custom_LOC.{ext}"
        if candidate.exists():
            return candidate
    raise FileNotFoundError(f"No temperature_custom_LOC.* in {data_dir}")


def _find_chronicle_file(data_dir: Path, station_id: str) -> Path | None:
    safe_id = safe_file_token(station_id)
    for pattern in (f"temperature_custom_{safe_id}_*.csv", f"temperature_custom_{station_id}_*.csv"):
        matches = sorted(data_dir.glob(pattern))
        if matches:
            return matches[0]
    exact = data_dir / f"temperature_custom_{safe_id}.csv"
