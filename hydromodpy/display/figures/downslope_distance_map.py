...
Which of the two it draws is written on the page in words, not left to a
symbol: the title names the cells the descent starts from and the network it
ends on, and the note under the map repeats both. A reader asking whether a
distance was measured on the observed network or on the simulated one should
never have to know what the second subscript of ``D_so`` stands for.
...
named in the legend. The support is cased in dark grey under its colours, so
which cells were measured at all reads before any class does: the lightest
class alone stands at a contrast of 1.3 against the ground, and a reader
cannot see a scale they cannot find.
...
class DistanceReading:
    """One of the two descents, named in words rather than in subscripts."""

    symbol: str
    """The name a trial publishes it under, ``D_so`` or ``D_os``."""

    origin: str
    """The cells the descent starts from, which is also the support averaged."""

    target: str
    """The network the descent has to reach for a length to exist."""

    @property
    def measures(self) -> str:
        """The sentence naming both ends, for a title or a note."""
        return f"how far each of {self.origin} descends before it reaches {self.target}"


DIRECTION_READINGS: dict[str, DistanceReading] = {
    "to_mapped": DistanceReading("D_so", "the simulated cells", "the mapped network"),
    "to_simulated": DistanceReading("D_os", "the mapped cells", "the simulated network"),
}
"""Per direction: what it measures, written so the symbol is never load-bearing."""
...
_OFF_SUPPORT_COLOR = GROUND_COLOR
_UNREACHABLE_COLOR = "#FFFFFF"
_UNREACHABLE_EDGE = "#333333"
_UNREACHABLE_HATCH = "///"

_SUPPORT_CASING_WEIGHT_PT = 2.0
"""Stroke of the casing under the support. Wider than a class of it."""

_OFF_SUPPORT_LABEL = "off support"
_SUPPORT_CASING_LABEL = "_support casing"
...
class DownslopeDistanceMap(BaseFigure):
    ...
    is ``D_so``, the descent of the simulated cells down to the mapped
    network, ``to_simulated`` is ``D_os``, the reverse. ``extent`` picks the
    frame: ``catchment`` crops to the delineated watershed, where both
    supports live, ``mesh`` keeps the whole domain.
    """

    spec = FigureSpec(
        name="downslope_distance_map",
        title="Downslope distance",
        kind="spatial",
    ...
    def render(
        ...
        **_,
        ...
        reading = DIRECTION_READINGS[_checked_direction(direction)]
        ...
        values = checked_cells(
            _distance_field(comparison, direction), n_faces, f"{reading.symbol} field"
        )
        support = checked_cells(
            _support(comparison, direction), n_faces, f"{reading.symbol} support"
        )
        ...
        draw_cells(
            ax,
            select_cells(polygons, ~on_support),
        ...
        draw_cells(
            ...
            select_cells(polygons, on_support),
            color=CASING_COLOR,
            label=_SUPPORT_CASING_LABEL,
            zorder=2,
            weight_pt=_SUPPORT_CASING_WEIGHT_PT,
        )
        for index, label in enumerate(labels):
            draw_cells(
                ax,
                select_cells(polygons, classes == index),
                color=colors[index],
                label=label,
                zorder=3 + index,
        ...
        draw_cells(
            ax,
            select_cells(polygons, unreachable),
        ...
        if unreachable.any():
            handles.append(
                Patch(
                    ...
                    hatch=_UNREACHABLE_HATCH,
                    label=(
                        f"unreachable: the descent never reaches {reading.target} "
        ...
        notes = [
            f"{reading.symbol}: {reading.measures}",
            f"support: {reading.origin} and no other, {cell_count(on_support)} averaged here",
            threshold_note(comparison),
            f"frame: {window.name}",
        ]
...
def _checked_direction(direction: str) -> str:
    ...
    if direction not in DIRECTION_READINGS:
        raise ValueError(
            f"direction must be one of {tuple(DIRECTION_READINGS)}, got {direction!r}."
        )
    return direction
