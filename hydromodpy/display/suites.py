...
This module mirrors :mod:`hydromodpy.display.orchestration` but is the
...
def _load_observed_streamflow(result) -> pd.DataFrame | None:
    ...
    from hydromodpy.display.adapters import observed_discharge_series
...
def plot_flow_suite(result, options: DisplayOptions) -> None:
    ...
    if options.flow.is_enabled("piezometry", default=True):
        from hydromodpy.display.adapters import observed_piezometry_series
