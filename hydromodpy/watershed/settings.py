"""Legacy watershed settings payload.

This settings object belongs to the preserved ``hydromodpy.watershed``
workflow. It is no longer part of the canonical simulation runtime, which now
derives model/run naming from ``simulation.run_id`` and explicit runtime
overrides instead.
"""
...
@dataclass
class Settings:
    """Mutable settings payload preserved for historical watershed scripts."""
