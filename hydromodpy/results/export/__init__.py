"""FAIR export formats for HydroModPy simulations and ML datasets.
...
- :mod:`hydromodpy.results.export.croissant`: Croissant MLCommons 1.0 manifest.
...
``stac-validator``, ``mlcroissant``) are optional and only loaded when
explicitly requested. The serialized payloads are always plain Python
``dict`` objects so callers can write them as JSON without third-party
dependencies.
"""
...
from hydromodpy.results.export.croissant import build_croissant_manifest, write_croissant
