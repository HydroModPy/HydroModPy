...
-- =====================================================================
-- Section 4.6 - metric_definitions (reference + ML-aware metadata)
-- =====================================================================
...
-- =====================================================================
-- Section 4.10bis - sim_configs / sim_config_paths (config-as-features)
--   STUB: DDL only; no production code populates these tables in v2.0.
-- =====================================================================
...
CREATE INDEX ix_sim_configs_schema ON sim_configs(schema_version);
CREATE INDEX ix_sim_configs_sha    ON sim_configs(config_sha256);

CREATE TABLE sim_config_paths (
    sim_id          UUID NOT NULL,
    path            VARCHAR NOT NULL,
    value_num       DOUBLE,
    value_text      VARCHAR,
    ...
    dtype           VARCHAR NOT NULL
                    CHECK (dtype IN ('int', 'float', 'str', 'bool', 'list', 'dict', 'null')),
    PRIMARY KEY (sim_id, path)
);

CREATE INDEX ix_sim_config_paths_path ON sim_config_paths(path);
CREATE INDEX ix_sim_config_paths_sim  ON sim_config_paths(sim_id);

-- =====================================================================
-- Section 4.10ter - feature_definitions (offline feature store)
--   STUB: DDL only; no production code populates this table in v2.0.
-- =====================================================================
...
CREATE TABLE feature_definitions (
    ...
    kind                VARCHAR NOT NULL
                        CHECK (kind IN (
                            'scalar', 'timeseries',
                            'spatial_2d', 'spatial_3d', 'config_path'
                        )),
    storage_kind        VARCHAR NOT NULL
                        CHECK (storage_kind IN (
                            'zarr', 'parquet',
                            'duckdb_column', 'sim_config_paths'
                        )),
    storage_path        VARCHAR NOT NULL,
    dtype               VARCHAR NOT NULL,
    unit                VARCHAR,
    standard_name_cf    VARCHAR,
    physical_min        DOUBLE,
    physical_max        DOUBLE,
    fill_value          DOUBLE,
    role                VARCHAR NOT NULL
                        CHECK (role IN ('feature', 'target', 'metadata', 'diagnostic')),
    description         VARCHAR,
    feature_references  VARCHAR,
    created_at          TIMESTAMPTZ NOT NULL DEFAULT current_timestamp
);

CREATE INDEX ix_feat_def_role ON feature_definitions(role);
CREATE INDEX ix_feat_def_kind ON feature_definitions(kind);

-- =====================================================================
-- Section 4.10quater - catalog_snapshots (local time-travel)
--   STUB: DDL only; no production code populates this table in v2.0.
-- =====================================================================
...
CREATE INDEX ix_cat_snap_parent  ON catalog_snapshots(parent_id);
CREATE INDEX ix_cat_snap_created ON catalog_snapshots(created_at DESC);

-- =====================================================================
-- Section 4.10 - ml_splits / ml_scalers / ml_datasets / ml_runs / ml_predictions
--   STUB: DDL only; no production code populates these tables in v2.0.
-- =====================================================================
...
CREATE TABLE ml_scalers (
    scaler_id       UUID PRIMARY KEY,
    strategy        VARCHAR NOT NULL
                    CHECK (strategy IN ('standard', 'minmax', 'robust', 'quantile')),
    feature_names   VARCHAR[] NOT NULL,
    state           BLOB NOT NULL,
    fingerprint     VARCHAR NOT NULL,
    fitted_on_split UUID,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT current_timestamp
);
...
CREATE TABLE ml_runs (
    ...
    framework       VARCHAR
                    CHECK (framework IS NULL OR framework IN (
                        'torch', 'jax', 'tensorflow', 'sklearn'
                    )),
    hyperparams     JSON,
    metrics         JSON,
    artifact_path   VARCHAR,
    tracking_uri    VARCHAR,
    git_commit      VARCHAR,
    python_version  VARCHAR,
...
CREATE TABLE ml_predictions (
    ...
    sim_id          UUID,
    split           VARCHAR
                    CHECK (split IS NULL OR split IN ('train', 'val', 'test', 'inference')),
