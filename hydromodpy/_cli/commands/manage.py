...
from hydromodpy._cli.commands.delete import delete_simulation_artifacts
from hydromodpy._cli.helpers import EXIT_CONFIG, resolve_workspace
...
def run(args: argparse.Namespace) -> None:
    ...
    if workspace_arg:
        ...
        scan_root = Path(scan_root_arg).expanduser().resolve() if scan_root_arg else Path.cwd().resolve()
...
class _WorkspaceManagerBackend:
    ...
    def preview_table(
        ...
        allowed = {
            entry["name"] for entry in self.list_tables(workspace_ref, database)["tables"]
        }
    ...
    def _discover_workspaces(scan_root: Path) -> list[Path]:
        roots = {
            path.parent.resolve()
            for path in scan_root.rglob("hydromodpy.duckdb")
            if path.is_file()
        }
        return sorted(roots)
