...
def setup_paths(root_dir, example_name, results_dir="results", env_var_name=None):
    ...
    out_path = os.getenv(env_var_name, os.path.join(root_dir, "examples_legacy", results_dir))
...
def load_simulation_results(simulations_folder, model_name, result_types=None):
    ...
    ...     simulations_folder, "test_0", result_types=["wte", "pathlines"]
    ... )
    """
    ...
    try:
        ...
        if "wte" in result_types:
            wte_path = os.path.join(
                model_path, "_postprocess/_rasters/watertable_elevation_t(0).tif"
            )
        ...
        if "wtd" in result_types:
            ...
            results["wtd_data"], results["wtd_rio"] = load_raster(
                wtd_path, mask_value=0, mask_operator="<="
            )

        if "seepage" in result_types:
            seep_path = os.path.join(model_path, "_postprocess/_rasters/seepage_areas_t(0).tif")
            results["seep_data"], results["seep_rio"] = load_raster(
                seep_path, mask_value=0, mask_operator="<="
            )

        # Load vectors
        if "pathlines" in result_types:
            pathlines_path = os.path.join(
                model_path, "_postprocess/_particles/pathlines_weighted.shp"
            )
...
def make_timeseries_data(start_date, end_date, freq, values, name="timeseries"):
    ...
    Examples:
    ---------
    >>> recharge = make_timeseries_data(
    ...     "2017-01-01", "2017-12-31", "ME", [10, 20, 15, ...], name="recharge"
    ... )
    """
