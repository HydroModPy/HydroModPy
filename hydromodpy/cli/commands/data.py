...
def register(subparsers) -> argparse.ArgumentParser:
    ...
    fetch = sub.add_parser(
        "fetch",
        help="Fetch an upstream variable and write a JSON sidecar next to the file",
    )
    fetch.add_argument("variable", help="Variable name (e.g. dem, piezometry, hydrometry)")
    fetch.add_argument(
        "--bbox",
        default=None,
...
def run(args: argparse.Namespace) -> None:
    ...
    if sub == "check":
        ...
        print(
            "Usage: hmp data {check|list|add|remove|prune|export|import|fetch} [options]",
            file=sys.stderr,
        )
...
def _cmd_fetch(args: argparse.Namespace) -> None:
    ...
    sidecar = Sidecar(
        ...
        crs=None,
        bbox=bbox,
        notes=f"auto-generated by 'hmp data fetch {spec.name}'",
    )
