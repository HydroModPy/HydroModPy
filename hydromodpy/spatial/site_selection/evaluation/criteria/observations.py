...
from hydromodpy.spatial.site_selection.evidence.station_influence import (
...
def evaluate_station_influence_criterion(
    *,
    ...
    blocking_issues, warning_issues = _station_influence_issues(
    ...
    failed = bool(blocking_issues)
    warned = bool(warning_issues)
    if failed:
        status = "failed"
    elif warned:
        status = "warning"
    ...
    return CriteriaComponent(
        ...
        blocking=failed and mode == "hard_reject",
        reason=(
            "; ".join([*blocking_issues, *warning_issues])
...
def _station_influence_issues(
    *,
    ...
    def _append_explicit_issue(message: str) -> None:
        if mode == "hard_reject":
            blocking_issues.append(message)
        else:
            warning_issues.append(message)

    if "general_influence" in flags and detail.warn_if_general_influence:
        _append_explicit_issue("station has general hydrologic influence metadata")
    elif "local_influence" in flags and detail.warn_if_local_influence:
        _append_explicit_issue("station has local hydrologic influence metadata")

    if detail.warn_if_comment_keyword and any(flag.endswith("_keyword") for flag in flags):
        if not warning_issues:
            warning_issues.append("station comments mention possible hydraulic influence")

    if not has_evidence and detail.unknown_policy == "warning":
        warning_issues.append("station influence metadata is missing")
    elif status == "unknown" and detail.unknown_policy == "warning":
        warning_issues.append("station influence status is unknown")
