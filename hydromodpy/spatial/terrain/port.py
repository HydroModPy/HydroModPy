...
"""The encoding of a pointer raster. ``d8_wbt`` is the one :data:`D8_WBT_OFFSETS` reads."""

D8_WBT_OFFSETS: dict[int, tuple[int, int]] = {
    1: (-1, 1),  # NE
    2: (0, 1),  # E
    4: (1, 1),  # SE
    8: (1, 0),  # S
...
"""What each ``d8_wbt`` code means, as a ``(drow, dcol)`` step to the receiving cell.

WhiteboxTools with ``esri_pntr=False`` stores, per cell, the power-of-two code
pointing at its single downslope neighbour, laid out clockwise from the
north-east::
...
``drow`` is counted downward because a raster's first row is its northernmost.
A pit, nodata, or any other code has no downstream cell and is absent here.
...
It lives beside the ``PointerConvention`` vocabulary that names it, and not in
the pipeline that first needed it, because a third-party engine has to read the
same table to serve the port without importing anything else of HydroModPy.
"""
