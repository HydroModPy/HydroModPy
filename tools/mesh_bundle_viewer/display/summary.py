...
def build_visualization_summary_contract(
    ...
    geology_keys = sorted(
        {str(cell.geology_key) for cell in mesh.cells if str(cell.geology_key).strip() != ""}
    )
    ...
    return VisualizationSummary(
        ...
        boundary_edge_count=int(sum(1 for edge in mesh.edges if str(edge.edge_kind) == "boundary")),
        geology_interface_edge_count=int(
            sum(1 for edge in mesh.edges if str(edge.edge_kind) == "geology_interface")
        ),
        ...
        topography_render_mode=(
            "continuous_on_nodes" if has_continuous_node_topography(mesh) else "fallback_on_cells"
        ),
