...
def _render_mesh_coverage_matrix(cases: list[dict[str, Any]]) -> list[str]:
    ...
    for scale in MESH_GALLERY_SCALE_ORDER:
        ...
        for variant in MESH_GALLERY_VARIANT_SPECS:
            ...
            if outlet_ids:
                outlet_suffix = ", ".join(
                    f"outlet {item}" for item in sorted(outlet_ids)
                )
    ...
    for case in cases:
        ...
        entry = families.setdefault(
            family_key,
            {
                ...
                "scale_label": str(
                    metadata.get("scale_label", metadata.get("scale", ""))
                ).strip(),
                "variant_entries": {},
            },
        )
        ...
        variant_label = str(
            metadata.get("variant_label", metadata.get("variant", ""))
        ).strip()
        if variant_label != "":
        ...
        if outlet_id != "" and variant_label != "":
            entry["variant_entries"][variant_key or variant_label]["outlet_ids"].append(
                outlet_id
            )
...
def _load_existing_case_summary(
    ...
    *,
    ...
    candidate_paths.extend(
        sorted(
            (source_root / "_static" / "capability_gallery").glob(
                f"*/{slug}_summary.json"
            )
        )
    )
...
def _cleanup_stale_case_artifacts(
    ...
    *,
    ...
    if old_summary is None:
        return
    stale_paths = _known_case_generated_paths(
        old_summary
...
def _should_preserve_previous_case_summary(
    ...
    generation_status = str(
        new_summary.get("metadata", {}).get("generation_status", "")
    ).strip()
    if generation_status == "source_unavailable_local":
        return True

    previous_images, previous_metrics, previous_artifacts = _summary_richness(
        previous_summary
    )
...
def _write_gallery_pages_from_summaries(
    *,
    ...
    for category_slug, cases in summaries_by_category.items():
        if (
            category_slugs_to_write is not None
            and category_slug not in category_slugs_to_write
        ):
            continue
        if cases:
            _write_text(
...
def _generate_selected_gallery(
    *,
    ...
    for spec in selected_specs:
        ...
        summary_path = (
            static_dir
            / str(case_summary["category"])
            / f"{case_summary['slug']}_summary.json"
        )
        _write_json(summary_path, case_summary)
        _write_text(
            case_dir / f"{case_summary['slug']}.rst", _build_case_page(case_summary)
        )
    ...
    _write_gallery_pages_from_summaries(
        ...
        rewrite_calibration_pages=any(
            spec.category == "calibration" for spec in selected_specs
        ),
    ...
    *,
    ...
    if images is None:
        ...
        for asset in spec.image_assets:
            static_doc_path = "/" + _docs_relative_static_path(
                spec.category, asset.filename
            )
            images.append(
                {
                    ...
                    "repo_path": _repo_docs_artifact_path(
                        spec.category, asset.filename
                    ),
                }
            )
...
def _generate_mesh_viewer_case(
    spec: GalleryCaseSpec, source_root: Path
) -> dict[str, Any]:
    preferred_assets = [
        asset for asset in spec.image_assets if asset.source_path is not None
    ]
    if preferred_assets:
        with _temporary_gallery_dir(
            prefix="hydromodpy_doc_gallery_mesh_viewer_"
        ) as temp_dir:
        ...
        for asset in preferred_assets:
            destination = source_root / _docs_relative_static_path(
                spec.category, asset.filename
            )
    ...
    if "bundle_dir" in raw_summary:
        sanitized_viewer_summary["bundle_dir"] = _repo_relative(
            str(raw_summary["bundle_dir"])
        )
    if "mesh_file" in raw_summary:
        sanitized_viewer_summary["mesh_file"] = _repo_relative(
            str(raw_summary["mesh_file"])
        )

    return _build_case_summary(
        spec,
        metrics_source={
            metric.key: raw_summary[metric.key] for metric in spec.metric_specs
        },
        metadata={
            **dict(spec.metadata),
...
def _generate_copy_assets_case(
    spec: GalleryCaseSpec, source_root: Path
) -> dict[str, Any]:
    copied_assets = []
    for asset in spec.image_assets:
    ...
    if static_summary_path:
        try:
            static_summary = json.loads(
                _repo_path(static_summary_path).read_text(encoding="utf-8")
            )
    ...
    *,
    ...
    for row in case_matrix_rows:
        ...
        if site_id and site_id not in site_order:
            ...
            site_labels[site_id] = (
                str(row.get("site_label", site_id)).strip() or site_id
            )
        if recipe_id and recipe_id not in recipe_order:
            recipe_order.append(recipe_id)
            recipe_labels[recipe_id] = (
                str(row.get("recipe_label", recipe_id)).strip() or recipe_id
            )
    ...
    if site_order and recipe_order:
        ...
        ax_matrix.set_yticklabels(
            [recipe_labels[recipe_id] for recipe_id in recipe_order]
        )
        ax_matrix.set_title(
            "Site x Recipe Matrix", loc="left", fontsize=12, fontweight="bold"
        )
    ...
    recipe_labels_ordered = [
        str(row.get("recipe_label", row.get("recipe_id", ""))).strip()
    ...
    if len(recipe_rows) > 0:
        ax_recipe.barh(
            y_positions, recipe_candidate, color="#E6E6E6", label="Candidate sites"
        )
        ...
        ax_recipe.set_title(
            "Recipe Coverage", loc="left", fontsize=12, fontweight="bold"
        )
    ...
    summary_lines = [
        ...
        ", ".join(
            f"{label} ({count})" for label, count in sorted(status_counts.items())
        )
        or "none",
        "",
        "Candidate site maturity mix:",
        ", ".join(
            f"{label} ({count})" for label, count in sorted(maturity_counts.items())
        )
        or "none",
        "",
        "Gap reasons:",
        ", ".join(f"{label} ({count})" for label, count in sorted(gap_counts.items()))
        or "none",
    ]
    if cluster_lines:
    ...
    fig.subplots_adjust(
        left=0.06, right=0.98, top=0.90, bottom=0.10, wspace=0.30, hspace=0.34
    )
        ...
        for key, value in payload.items():
            if key == "generated_at_utc":
                ...
                sanitized[key] = {
                    str(path_key): (
                        (
                            Path("regional_lab_outputs") / Path(path_value).name
                        ).as_posix()
...
def _generate_regional_lab_case(
    spec: GalleryCaseSpec, source_root: Path
) -> dict[str, Any]:
    ...
    with _temporary_gallery_dir(
        prefix="hydromodpy_doc_gallery_regional_lab_"
    ) as temp_dir:
        ...
        sanitized_report_payload = _sanitize_regional_lab_artifact_payload(
            report_payload
        )
        ...
        static_dir = (
            source_root / Path("_static") / "capability_gallery" / spec.category
        )
        static_dir.mkdir(parents=True, exist_ok=True)
    ...
    metadata = {
        **dict(spec.metadata),
        ...
        "recipe_labels": {
            ...
            .strip(): str(row.get("recipe_label", ""))
            .strip()
            for row in recipe_rows
            if str(row.get("recipe_id", "")).strip()
        },
...
def _generate_validation_case(
    ...
    comparison_runner = getattr(
        run_case_module, str(spec.metadata["comparison_function_name"])
    )
    plotting_function = getattr(
        run_case_module, str(spec.metadata["plotting_function_name"])
    )
    ...
    solver_variants = tuple(
        str(item) for item in spec.metadata.get("solver_variants", ())
    )
    default_solver = str(
        spec.metadata.get(
            "default_solver", solver_variants[0] if solver_variants else ""
        )
    )
    solver_details = {
        str(key): value
        for key, value in dict(spec.metadata.get("solver_details", {})).items()
    }
...
def _generate_xt3d_method_choice_case(
    spec: GalleryCaseSpec, source_root: Path
) -> dict[str, Any]:
    ...
    figure_path = source_root / _docs_relative_static_path(
        spec.category, figure_asset.filename
    )
    render_xt3d_tradeoff_figure(payload, figure_path)

    report_filename = f"{spec.slug}_report.json"
    report_path = source_root / _docs_relative_static_path(
        spec.category, report_filename
    )
    ...
    strongest_row = max(
        payload["rows"], key=lambda row: float(row["rmse_improvement_factor"])
    )
...
def _generate_calibration_case(
    spec: GalleryCaseSpec, source_root: Path
) -> dict[str, Any]:
        ...
        },
        {
            ...
            "display": (
                ...
                if display_result.calibration_time_seconds is None
                    ...
                    display_result.calibration_time_seconds, "s"
                )
            ),
        },
        {
            ...
            "display": (
                ...
                if display_result.session_prepare_time_seconds is None
                    ...
                    display_result.session_prepare_time_seconds, "s"
                )
            ),
        },
        {
        ...
        },
        {
            ...
            "display": (
                ...
                if display_result.mean_candidate_total_time_seconds is None
                    ...
                    display_result.mean_candidate_total_time_seconds, "s"
                )
            ),
        },
        {
    ...
    for result in method_results:
        method_assets = method_assets_by_instance.get(
            str(result.method_instance_name), {}
        )
        posterior_filename = (
        ...
        method_run_row = {
            ...
            "model_distribution_sample_count": int(
            ...
            "params_best": {
                str(name): float(value) for name, value in result.params_best.items()
            },
...
def _load_committed_simulation_comparison_payload(
    ...
    if not (
        manifest_path.exists() and metrics_path.exists() and observables_path.exists()
    ):
        return None
...
def _build_simulation_comparison_payload(
    ...
    committed_payload = _load_committed_simulation_comparison_payload(
        cfg.comparison_root
    )
    ...
    for simulation in section.simulation:
        ...
        if run_folder is None:
            committed_run_folder = str(
                committed_simulation.get("run_folder", "")
            ).strip()
        ...
        simulation_summaries.append(
            {
                ...
                "metrics": compact_run_metrics(
                    read_json_file(run_folder / "_metrics.json")
                ),
...
def _align_single_snapshot_rows(rows: list[dict[str, Any]]) -> None:
    ...
    for row in rows:
        ...
        snapshot_keys.setdefault(key, set()).add(
...
def _render_simulation_comparison_figure(
    *,
    ...
    ax_bar.set_xticklabels(
        [_humanize_case_token(str(row["observable"])) for row in summary_rows]
    )
...
def _generate_square_parameterizations_property_case(
    ...
    structured_mesh = FieldMeshSquare.from_unit_square(
        target_n_cells=64, mesh_kind="structured"
    )
    ...
    for label, parameter, linestyle in profiles:
        profile_values = (
            np.asarray(parameter.to_array(depth=depths), dtype=float) * 86400.0
        )
        ax_profile.plot(
            depths, profile_values, linestyle=linestyle, linewidth=2.0, label=label
        )
    ...
    for identifier, raw_value, parameter in unit_examples:
        ...
        unit_lines.append(
            f"{identifier:<2} {raw_value:<12} -> {normalized:.3e} {parameter.unit}"
        )
    ax_text.text(
        0.02,
        0.98,
    ...
    _add_horizontal_colorbar(
        fig, image_handle, axes=[ax_struct, ax_tri], label="K [m/day]"
    )
...
def _generate_depth_dependence_property_case(
    ...
    deep_values = (
        np.asarray(
            exponential_param.to_mesh_field(
                field.on_mesh(mesh), depth=depth_m
            ).cell_values,
            dtype=float,
        )
        * 86400.0
    ...
    tab_profile_values = (
        np.asarray(tabulated_profile.to_array(depth=depths), dtype=float) * 86400.0
    )
    ax_profile.plot(depths, exp_profile_values, linewidth=2.0, label="Exponential")
    ax_profile.plot(
        depths, tab_profile_values, linewidth=2.0, linestyle="--", label="Tabulated"
    )
    ...
    _add_horizontal_colorbar(
        fig, image_handle, axes=[ax_surface, ax_deep], label="K [m/day]"
    )
...
def _render_geology_transfer_demo(
    *,
    ...
    loaded, gdf, _window_polygon, sea_info = demo._load_display_geology(
        args, geology_config_path
    )
    ...
    _add_horizontal_colorbar(
        fig, image_handle, axes=[ax_left, ax_right], label="K [m/day]"
    )
    fig.subplots_adjust(top=0.9)
    fig.savefig(figure_path, bbox_inches="tight")
    plt.close(fig)
...
def _render_geology_bundle_variant(
    *,
    ...
    geology_idx = np.asarray(
        [key_to_idx.get(key, -1) for key in geology_keys_arr], dtype=float
    )
    ...
    return {
        ...
        "property_max": (
            float(np.nanmax(k_values_arr)) if k_values_arr.size else float("nan")
        ),
    }
...
def _generate_geology_transfer_variants_property_case(
    ...
    if metrics_reference is None:
        raise ValueError(
            "No variant metrics were produced for geology_transfer_variants."
        )
...
def _generate_mesh_constraint_balance_case(
    ...
    if not entries:
        raise ValueError(
            "mesh_constraint_balance_case requires bundle_entries metadata."
        )
    ...
    *,
    ...
    if points.size == 0:
        ...
        for flat_idx in flat_indices:
            ...
            if centers:
                distances = [
                    np.linalg.norm(candidate - np.array(center)) for center in centers
                ]
...
def _generate_mesh_zoom_case(
    ...
    for edge in bundle.edges:
        ...
        if edge.is_river:
            ...
            river_midpoints.append(
                [(seg[0, 0] + seg[1, 0]) * 0.5, (seg[0, 1] + seg[1, 1]) * 0.5]
            )
        if str(edge.geology_b_key).strip() and str(edge.geology_a_key).strip():
            if str(edge.geology_a_key).strip() != str(edge.geology_b_key).strip():
    ...
    if interface_segments:
        ax_full.add_collection(
            LineCollection(
                interface_segments, colors="#5c7cfa", linewidths=0.5, alpha=0.8
            )
        )
    if river_segments:
        ax_full.add_collection(
    ...
    for ax, bounds in zip(zoom_axes, zoom_bounds):
        ...
        if interface_segments:
            ax.add_collection(
                LineCollection(
                    interface_segments, colors="#5c7cfa", linewidths=0.7, alpha=0.85
                )
            )
        if river_segments:
            ax.add_collection(
                LineCollection(
                    river_segments, colors="#1f78b4", linewidths=1.0, alpha=0.95
                )
            )
...
def _load_geometry_layers(metadata: dict[str, Any]):
    ...
    target_crs = (
        boundary.crs or rivers.crs or (geology.crs if geology is not None else None)
    )
    ...
    if geology is not None:
        geology_clipped = (
            gpd.clip(geology, boundary_geom) if not geology.empty else geology
        )
...
def _read_dem_for_plot(dem_path: Path, *, max_dim: int = 1200):
    ...
    with rasterio.open(str(dem_path)) as src:
        scale = max(
            float(src.width) / float(max_dim), float(src.height) / float(max_dim), 1.0
        )
    ...
    if geometry_case_kind == "constraints_overview":
        ...
        if geology is not None and not geology.empty:
            ...
            if "zone_key" in plot_geology.columns:
                plot_geology["zone_idx"] = (
                    plot_geology["zone_key"].astype("category").cat.codes
                )
            else:
        ...
        if show_context and ax_context is not None:
            dem, extent = _read_dem_for_plot(
                _repo_path(str(dem_path_value)), max_dim=1400
            )
            ...
            fig.colorbar(
                im, ax=ax_context, shrink=0.75, pad=0.02, label="Elevation [m]"
            )
        ...
        fig.colorbar(
            image_handle, ax=ax_dem, shrink=0.75, pad=0.02, label="Elevation [m]"
        )

        slope_handle = ax_slope.imshow(
            slope, extent=extent, cmap="magma", origin="upper"
        )
        ...
        fig.colorbar(
            slope_handle, ax=ax_slope, shrink=0.75, pad=0.02, label="Slope [deg]"
        )
        ...
        im_dem = ax_dem.imshow(
            dem_masked, extent=extent, cmap="terrain", origin="upper"
        )
...
def _generate_simulation_comparison_case(
    ...
    if config_path.exists():
        manifest, metrics_payload, all_rows = _build_simulation_comparison_payload(
            config_path
        )
    else:
    ...
    write_metrics_csv(
        summary_csv_path, focus_summary_rows, fieldnames=SUMMARY_METRIC_FIELDS
    )
    write_metrics_csv(
        differences_csv_path, focus_detail_rows, fieldnames=DETAIL_METRIC_FIELDS
    )
        ...
        extra_summary={
            "artifacts": {
                ...
                "extra_repo_paths": [
                    ...
                    _repo_docs_artifact_path(
                        spec.category, f"{spec.slug}_comparison_metrics.json"
                    ),
                    _repo_docs_artifact_path(
                        spec.category, f"{spec.slug}_observables.csv"
                    ),
                    _repo_docs_artifact_path(
                        spec.category, f"{spec.slug}_summary_metrics.csv"
                    ),
                    _repo_docs_artifact_path(
                        spec.category, f"{spec.slug}_difference_metrics.csv"
                    ),
                ],
            },
        },
...
def _append_custom_sections(
    lines: list[str], custom_sections: list[dict[str, Any]]
) -> None:
...
def _build_validation_grouped_sections(
    ...
    for case in cases:
        ...
        family_label = str(
            metadata.get(
                "validation_family_label", family_key.replace("_", " ").title()
            )
        ).strip()
    ...
    for case in calibration_cases:
        ...
        entry = grouped.setdefault(
            family_key,
            {
                "key": family_key,
                "title": str(
                    metadata.get("benchmark_family_title", family_spec["title"])
                ),
...
def _generate_calibration_intercomparison_summary(
    *,
    ...
    summary_relative_path = (
        output_root / "calibration_intercomparison_summary.json"
        ...
        for row in rows:
            ...
            lines.extend(
                [
                    ...
                    + (
                        ""
                        if row.get("mean_candidate_launcher_prepare_time_seconds")
                        is None
                        else _format_metric_display(
                    ...
                    + (
                        ""
                        if row.get("mean_candidate_output_selection_time_seconds")
                        is None
                        else _format_metric_display(
                    ...
                    + (
                        ""
                        if row.get("mean_candidate_objective_compute_time_seconds")
                        is None
                        else _format_metric_display(
    ...
    for _, group_cases in sorted(
        ...
        ordered_cases = sorted(
            ...
            key=lambda case: (
                ...
                _mesh_case_sort_token(
                    str(case.get("metadata", {}).get("outlet_id", ""))
                ),
                str(case.get("title", "")),
            ),
        )
...
def _group_mesh_cases_for_tabs(
    ...
    for group_key, group_cases in sorted(
        ...
        key=lambda item: (
            scale_rank.get(
                str(item[1][0].get("metadata", {}).get("scale", "")).strip(), 999
            ),
            _mesh_case_sort_token(
                str(item[1][0].get("metadata", {}).get("outlet_id", ""))
            ),
        ...
        ordered_cases = sorted(
            group_cases,
            key=lambda case: _variant_rank(
                str(case.get("metadata", {}).get("variant", ""))
            ),
        )
        if len(ordered_cases) > 1:
            metadata = dict(ordered_cases[0].get("metadata", {}))
    ...
    if category_slug == "mesh":
                ...
                + "; ".join(
                    str(
                        case.get("metadata", {}).get(
                            "variant_label",
                            case.get("metadata", {}).get(
                                "variant", case.get("title", "Variant")
                            ),
                        )
                    )
        ...
        lines.extend(
            [
                ...
                "- Imported scales: "
                + (
                    ", ".join(imported_scale_labels)
                    if imported_scale_labels
                    else "none yet"
                )
                + ".",
                "- Present variants: "
        ...
        lines.extend(
            family_matrix if family_matrix else _render_mesh_coverage_matrix(cases)
        )
        if site_tab_groups:
            lines.extend(
                [
            ...
            for group in site_tab_groups:
                ...
                for case in group["cases"]:
                    ...
                    outlet_id = str(
                        case.get("metadata", {}).get("outlet_id", "")
                    ).strip()
                    if outlet_id:
                    ...
                    if constraints_mode:
                        lines.append(
                            f"      - Constraints mode: ``{constraints_mode}``"
                        )
        ...
        if tabbed_groups:
            ...
            for group in tabbed_groups:
                ...
                for case in group["cases"]:
                    variant_label = str(
                        case.get("metadata", {}).get(
                            "variant_label", case.get("title", "Variant")
                        )
                    )
                    lines.extend(
                        [
        ...
        for case in cases:
            ...
            family_counts[
                (
                    ...
                    str(metadata.get("validation_family_label", "Other")).strip()
                    or "Other",
                )
            ] += 1
            reference_counts[
                (
                    ...
                    str(metadata.get("reference_type_label", "Other")).strip()
                    or "Other",
                )
            ] += 1
        ...
        if source_families:
            lines.append(
                "- Explicit source families: " + ", ".join(source_families) + "."
            )
        lines.extend(
            [
        ...
        lines.extend(
            [
                ...
                "",
                "- Geometry layers: "
                + (", ".join(layers) if layers else "none yet")
                + ".",
        ...
        lines.extend(
            [
                ...
                + (
                    ", ".join(parameterization_modes)
                    if parameterization_modes
                    else "none yet"
                )
                + ".",
                "- Spatial supports: "
                + (", ".join(supports) if supports else "none yet")
                + ".",
                "",
            ]
        )
        ...
        process_families = sorted(
            {
                ...
                for case in cases
                for process_family in case.get("metadata", {}).get(
                    "process_families", []
                )
        ...
        transport_solvers = sorted(
            {
                ...
                for case in cases
                for transport_solver in case.get("metadata", {}).get(
                    "transport_solvers", []
                )
        ...
        postprocess_outputs = sorted(
            {
                ...
                for case in cases
                for output_name in case.get("metadata", {}).get(
                    "postprocess_outputs", []
                )
        ...
        lines.extend(
            [
                ...
                "",
                "- Study areas: "
                + (", ".join(study_areas) if study_areas else "none yet")
                + ".",
        ...
        if postprocess_outputs:
            lines.append(
                "- Postprocess outputs surfaced: "
                + ", ".join(postprocess_outputs)
                + "."
            )
        lines.extend(
            [
        ...
        simulation_labels = sorted(
            {
                str(label)
                for case in cases
                for label in case.get("metadata", {})
                .get("simulation_labels", {})
                .values()
            }
        )
        ...
        lines.extend(
            [
                ...
                + (
                    ", ".join(conductivity_levels)
                    if conductivity_levels
                    else "none yet"
                )
                + ".",
                "- Solver families: "
        ...
        supplementary_families = [
            ...
            for group in family_groups
            if not (
                bool(group.get("primary")) and str(group.get("page_slug") or "").strip()
            )
        ]
        regimes = sorted(
            {
        ...
        lines.extend(
            [
                ...
                "",
                "- Solvers: "
                + (", ".join(solver_names) if solver_names else "none yet")
                + ".",
    ...
    for tab in tab_specs:
        ...
        if tab.get("filenames") is not None:
            ...
            if filename != "":
                filenames = [filename]
        images = [
            image_map[filename] for filename in filenames if filename in image_map
        ]
...
def _render_validation_solver_block(
    run: dict[str, Any], *, indent: str = ""
) -> list[str]:
    lines: list[str] = []
    image = run.get("image")
    if image:
...
def _build_geographic_parameter_docs(case: dict[str, Any]) -> dict[str, Any]:
    ...
    if not config_path:
        config_path = _find_first_source_path(
            case, suffix=".toml", contains="hydrography_only"
        )
    ...
    if hydrography_sources:
        first_source = (
            hydrography_sources[0] if isinstance(hydrography_sources[0], dict) else {}
        )
        _add_parameter_row(
            hydrography_rows,
    ...
    if hydrography_rows:
        sections.append(
            {"title": "Hydrography Source Options", "rows": hydrography_rows}
        )
    if panel_rows:
...
def _build_simulation_parameter_docs(case: dict[str, Any]) -> dict[str, Any]:
    ...
    _add_parameter_row(
        ...
        value=_lookup_path(
            payload, "flow", "param", "Sy", "field_homogeneous", "value"
        ),
        source=run_config_path or "",
    )
...
def _build_simulation_comparison_parameter_docs(case: dict[str, Any]) -> dict[str, Any]:
    config_path = str(
        case.get("metadata", {}).get("comparison_config_path", "")
    ).strip()
    if not config_path:
...
def _build_mesh_parameter_docs(case: dict[str, Any]) -> dict[str, Any]:
    ...
    if bundle_metrics_rows:
        sections.append(
            {"title": "Compared Bundle Values", "rows": bundle_metrics_rows}
        )
    return {"sections": sections} if sections else {}
...
def _build_property_parameter_docs(case: dict[str, Any]) -> dict[str, Any]:
    ...
    if not field_param_path:
        field_param_path = (
            _find_first_source_path(case, suffix="field_param_config.toml") or ""
        )
    ...
    if field_param_cfg:
        ...
        heterogeneous_values = _lookup_path(
            field_param_cfg, "field_heterogeneous", "values"
        )
        ...
        csv_file = _lookup_path(
            field_param_cfg, "field_heterogeneous", "values_csv_file"
        )
        _add_parameter_row(
            config_rows,
...
def _build_calibration_parameter_docs(case: dict[str, Any]) -> dict[str, Any]:
    ...
    for method_row in metadata.get("method_runs", []):
        ...
        if method_returns_parameter_distribution(method_name):
            value["posterior_samples"] = method_row.get(
                "model_distribution_sample_count"
            )
        _add_parameter_row(
            method_rows,
            field=str(
                method_row.get("method_instance_name", method_row.get("method_name"))
            ),
            meaning=(
...
def _normalize_parameter_sections(case: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if reference_parameters:
        normalized.append(
            {"title": "Reference Parameters", "rows": reference_parameters}
        )
    if common_setup_parameters:
        normalized.append(
            {"title": "Common Numerical Setup", "rows": common_setup_parameters}
        )
    ...
    if common_acceptance_rows:
        normalized.append(
            {"title": "Acceptance Criteria", "rows": common_acceptance_rows}
        )
...
def _build_case_page(case: dict[str, Any]) -> str:
    ...
    if walkthrough_doc:
        walkthrough_title = str(
            case.get("walkthrough_title") or "the guided walkthrough"
        )
    ...
    if not solver_runs:
        ...
        lead_image_filenames = [
            ...
            for filename in list(
                case.get("metadata", {}).get("lead_image_filenames", [])
            )
            if str(filename).strip()
        ]
        ...
        for section in parameter_sections:
            ...
            if tabs:
                _append_parameter_tabs(
                    ...
                    empty_message=str(
                        section.get(
                            "empty_message", "No additional case-specific parameters."
                        )
                    ),
                )
        if lines[-1] != "":
...
def _load_validation_batch_reports() -> list[dict[str, Any]]:
    ...
    for path in sorted(VALIDATION_REPORT_DIR.glob("*.json")):
        try:
            ...
            continue
        if (
            str(payload.get("schema_version", "")).strip()
            != "validation_batch_report_v1"
        ):
            continue
    ...
    solver_rank = {
        solver: index for index, solver in enumerate(VALIDATION_SOLVER_ORDER)
    }
    return sorted(
        reports,
        key=lambda payload: (
    ...
    with _temporary_gallery_dir(prefix="hydromodpy_doc_gallery_baseline_") as temp_dir:
        ...
        for category_slug in CATEGORY_SPECS:
            summaries_by_category[category_slug] = [
                ...
                if summary["category"] == category_slug
            ]

        _write_text(docs_dir / "index.rst", _build_index_page(summaries_by_category))
        calibration_cases = list(summaries_by_category.get("calibration", []))
        if calibration_cases:
            for family_group in _group_calibration_cases_by_benchmark_family(
                calibration_cases
            ):
                page_slug = str(family_group.get("page_slug") or "").strip()
                if page_slug == "":
                ...
                _write_text(
                    ...
                    _build_calibration_intercomparison_page(
                        ...
                        comparison_summary=comparison_summary,
                        title=str(
                            family_group.get("page_title") or family_group["title"]
                        ),
                        intro=str(
                            family_group.get("page_intro") or family_group["deck"]
                        ),
                        seealso_doc="calibration",
                        seealso_title="the calibration benchmark category",
                    ),
    ...
    if args.check:
        if only_slugs or categories:
            with _temporary_gallery_dir(prefix="hydromodpy_doc_gallery_") as temp_dir:
                ...
                new_summaries_by_slug, old_summaries_by_slug = (
                ...
                if issues:
                    print(
                        "Capability gallery selection is out of date. Regenerate it with:"
                    )
                    print(
