...
def _read_text(path: Path, *, encoding: str = "utf-8") -> str:
    return _filesystem_path(path).read_text(encoding=encoding)


def _open_file(path: Path, mode: str, **kwargs: Any) -> IO[Any]:
    return _filesystem_path(path).open(mode, **kwargs)
...
def _is_dir(path: Path) -> bool:
    return _filesystem_path(path).is_dir()


def _sorted_glob(path: Path, pattern: str) -> list[Path]:
    """Match ``pattern`` under ``path`` and give the matches back unprefixed.

    ``Path.glob`` swallows the OSError a too-long directory raises and yields
    nothing, so a bare glob reads an unreadable tree as an empty one.
    """
    scan_root = _filesystem_path(path)
    return [
        path.joinpath(*match.relative_to(scan_root).parts)
        for match in sorted(scan_root.glob(pattern))
    ]
...
def _load_existing_case_summary(
    ...
    *,
    ...
    candidate_paths.extend(
        _sorted_glob(source_root / "_static" / "capability_gallery", f"*/{slug}_summary.json")
    )
    for candidate_path in candidate_paths:
        if not _path_exists(candidate_path):
            continue
        try:
            payload = json.loads(_read_text(candidate_path))
    ...
    *,
    ...
    for relative_path in _summary_artifact_relative_paths(summary):
        ...
        if not _is_file(source_path):
            continue
        _make_dir(destination_path.parent)
        if (
            ...
            _write_json(
                destination_path,
                sanitize_public_comparison_manifest(json.loads(_read_text(source_path))),
            )
            continue
...
def _merge_case_summaries_by_category(
    ...
    *,
    ...
    if _is_dir(static_root):
        for summary_path in _sorted_glob(static_root, "*/*_summary.json"):
            try:
...
def _compare_selected_generated_tree(
    *,
    ...
    for relative_path, expected_path in expected_files.items():
        ...
        if not _path_exists(committed_path):
    ...
    for relative_path in sorted(stale_relative_paths or set()):
        if _path_exists(committed_source_root / relative_path):
...
def _sha256(path: Path) -> str:
    """Hash a provenance source, ignoring the checkout line endings.

    Git rewrites text files to CRLF on a Windows checkout, so hashing the raw
    bytes would record a different digest for the same commit depending on the
    platform. Sniffing for a NUL byte mirrors the heuristic Git itself uses to
    decide whether a blob is text, so binary sources keep their exact bytes.
    """
    content = _read_bytes(path)
    if b"\x00" not in content[:BINARY_SNIFF_BYTES]:
...
def _missing_repo_paths(paths: tuple[str, ...] | list[str]) -> list[str]:
    ...
    for relative_path in paths:
        ...
        if normalized == "" or normalized in seen:
            continue
        seen.add(normalized)
        if not _path_exists(_repo_path(normalized)):
...
def _remove_tree(path: Path) -> None:
    """Remove one directory tree, retrying under the long-path spelling."""
    ...
    try:
        ...
        if (
            ...
            shutil.rmtree(_filesystem_path(path), onerror=_rmtree_onerror)
            return
        raise
...
def _generate_copy_assets_case(spec: GalleryCaseSpec, source_root: Path) -> dict[str, Any]:
    ...
    if spec.category == "simulation_comparison":
        for source_path in spec.source_paths:
            ...
            _write_json(
                destination,
                sanitize_public_comparison_manifest(json.loads(_read_text(source_manifest_path))),
            )
    ...
    if static_summary_path:
        try:
            static_summary = json.loads(_read_text(_repo_path(static_summary_path)))
...
def _read_csv_rows(path: Path) -> list[dict[str, str]]:
    with _open_file(path, "r", encoding="utf-8", newline="") as handle:
...
def _render_regional_lab_figure(
    *,
    ...
    _make_dir(figure_path.parent)
    fig.savefig(_filesystem_path(figure_path), bbox_inches="tight")
    plt.close(fig)
...
def _generate_regional_lab_case(spec: GalleryCaseSpec, source_root: Path) -> dict[str, Any]:
    ...
    with _temporary_gallery_dir(prefix="hydromodpy_doc_gallery_regional_lab_") as temp_dir:
        ...
        plan_payload = json.loads(_read_text(plan_path))
        report_payload = json.loads(_read_text(report_path))
...
def _copy_generated_gallery_image(
    *,
    ...
    if source_path is None or not _is_file(source_path):
        return False
    _make_dir(destination.parent)
...
def _load_committed_simulation_comparison_payload(
    ...
    if not (_is_file(manifest_path) and _is_file(metrics_path) and _is_file(observables_path)):
        return None

    manifest = json.loads(_read_text(manifest_path))
    metrics_payload = json.loads(_read_text(metrics_path))
    with _open_file(observables_path, "r", encoding="utf-8", newline="") as handle:
...
def _build_simulation_comparison_payload(
    ...
    raw_toml = load_toml_with_base_config(_filesystem_path(config_path))
    ...
    if section.run_simulations and not _path_exists(committed_manifest_path):
        raise ValueError(
    ...
    if _path_exists(committed_manifest_path):
        committed_manifest = read_json_file(_filesystem_path(committed_manifest_path))
    ...
    for simulation in section.simulation:
        ...
        if not _path_exists(run_folder):
            raise FileNotFoundError(
        ...
        simulation_summaries.append(
            {
                ...
                "metrics": compact_run_metrics(
                    read_json_file(_filesystem_path(run_folder / "_metrics.json"))
                ),
...
def _generate_simulation_comparison_case(
    ...
    if _path_exists(config_path):
        ...
        for source_path in spec.source_paths:
            ...
            if not _path_exists(comparison_root):
                continue
    ...
    write_metrics_csv(
        _filesystem_path(summary_csv_path), focus_summary_rows, fieldnames=SUMMARY_METRIC_FIELDS
    )
    extra_repo_paths = [
    ...
    if publish_full_artifacts:
        ...
        write_public_observables_csv(_filesystem_path(observables_path), all_rows)
        write_metrics_csv(
            _filesystem_path(differences_csv_path),
            focus_detail_rows,
...
def _load_plain_toml(relative_path: str | None) -> dict[str, Any]:
    if not relative_path:
        return {}
    try:
        return tomllib.loads(_read_text(_repo_path(relative_path), encoding="utf-8-sig"))
...
def _collect_generated_files(source_root: Path) -> dict[str, Path]:
    ...
    for relative_dir in (
        ...
        if not _is_dir(target_dir):
            continue
        for path in _sorted_glob(target_dir, "**/*"):
            if _is_file(path):
...
def generate_gallery(*, source_root: Path) -> None:
    ...
    with _temporary_gallery_dir(prefix="hydromodpy_doc_gallery_baseline_") as temp_dir:
        ...
        _make_dir(baseline_source_root)
        baseline_static_dir = source_root / "_static" / "capability_gallery"
        if _is_dir(baseline_static_dir):
            shutil.copytree(
                _filesystem_path(baseline_static_dir),
                _filesystem_path(baseline_source_root / "_static" / "capability_gallery"),
                dirs_exist_ok=True,
            )
