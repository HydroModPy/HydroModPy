...
from hydromodpy.legacy.watershed import Driasclimat, Driaseau, \
    Geology, Hydraulic, Hydrography, Hydrometry, Piezometry, Settings, SafranSurfex, Transport
...
dem_path_pyhelp = era5_folder / "ursa_RS3_rot0_250.tif"
shapefile_path = era5_folder / "watershed_urse_EPSG2056.shp"
...
#%% Run
k_values = [4.28e-8 * 3600 * 24]
list_of_sims = []
option = "1"  # '1': update grid from DEM+shp, '2': grid already ready
...
dem_ref_path = Path(stable_folder) / "geographic" / "watershed_box_buff_dem.tif"

ds = xr.open_dataset(nc_path)
dem_ref = rxr.open_rasterio(dem_ref_path)

R = ds["rechg"]
R = R.rio.write_crs(dem_ref.rio.crs)

Rt = R.rio.reproject_match(dem_ref, nodata=0.0)
cube = Rt.values / 1000.0

ds.close()
dem_ref.close()

recharge_dict = {i: cube[i] for i in range(cube.shape[0])}
recharge_list = np.nanmean(Rt.values / 1000.0, axis=(1, 2)).tolist()
"""
#%% ---- RECHARGE
...
# climatic.update_recharge(values = recharge_dict, sim_state="steady")
...
iter_results = MatchingStreams(initializing, geographic, hydrography, 
                               iteration_label=model_name, from_calib=False)
