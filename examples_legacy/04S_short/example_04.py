...
#%% PERSONAL
example_path = os.path.join(root_dir, "examples_legacy", "04S_short/")
...
from_xyv = [391502.195, 6821197.683, 150, 10 , 'EPSG:2154'] # [x, y, snap distance, buffer size, crs proj]
catch_def = "xy"
...
########## LOOP ##########
list_porosity = np.array([0.5, 5]) / 100 # [-]
...
for i, simul in enumerate(simul_list[:]):
    ...
    with rasterio.open(os.path.join(stable_folder, 'geographic', 'watershed_dem.tif')) as src:
        mask = src.read(1)
    ...
    for i, key in enumerate([argmin, argmax]):
        ...
        with rasterio.open(BV.geographic.watershed_dem) as src:
            dem_data = src.read(1)
...
with rasterio.open(os.path.join(stable_folder, 'geographic', 'watershed_contour.tif')) as src:
    line = src.read(1)
        ...
        try:
            ...
            wbt.vector_lines_to_raster(path_sub,
                                       ...
                                       base = os.path.join(stable_folder,
                                                           'geographic',
                                                           'watershed_dem.tif'))
            with rasterio.open(os.path.join(glob.glob(
                os.path.join(stable_folder, 'subbasin', 'intermittency*'))[0],
                'watershed_contour.tif')) as src:
                line_sub = src.read(1)
...
with rasterio.open(os.path.join(stable_folder,
                                'geographic',
                                'watershed_contour.tif')) as src:
    line = src.read(1)
line = np.ma.masked_where(line <= 0, line)
...
for i, simul in enumerate(simul_list[:]):
    ...
    with rasterio.open(os.path.join(simul, r'_postprocess/_rasters',
                                    'persistency_index_t(-).tif')) as src:
        pi = src.read(1)
...
Qobs = Qobs.resample('ME').sum() * 1000 # m/day to mm/month
...
for i, simul in enumerate(simul_list[:]):
    ...
    axb.set_ylim(0,999)
    axb.invert_yaxis()
    axb.set_yticks([0, 200])
    axb.set_yticklabels([0, 200])

    Qobs_stat = select_period(Qobs, 2000, 2001)
    Qmod_stat = select_period(Qmod, 2000, 2001)
