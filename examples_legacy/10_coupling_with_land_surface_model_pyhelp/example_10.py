...
data_path = os.path.join(DIR, "examples_legacy", "10_coupling_with_land_surface_model_pyhelp", "data")

# The folder out_path is created in the example_path root directory:
out_path = os.path.join(DIR, "examples_legacy", "results")
...
k_values = [4.28e-8 * 3600 * 24] #â—˜ 0.0037 m/day
...
# Extraire les annÃ©es pour l'axe X
years = rec_annual.index.year
rec_annual.index = years  # Remplacer l'index par les annÃ©es pures
...
plt.xlabel('Years')
# plt.yscale('log')  # Optionnel selon Ã©chelle
plt.grid(True, which='both', linestyle='--', alpha=0.5)
plt.title('Yearly recharge [mm/year]')

# Mettre les annÃ©es en xticks proprement
...
# Ajouter colonnes annÃ©e et mois
rec_mean['year'] = rec_mean['time'].dt.year
rec_mean['month'] = rec_mean['time'].dt.month

# SÃ©lectionner uniquement les colonnes numÃ©riques Ã  sommer
cols_to_sum = ['rechg']

# Grouper par annÃ©e et mois, et sommer
rec_monthly = rec_mean.groupby(['year', 'month'], as_index=False)[cols_to_sum].sum()

# CrÃ©er une colonne datetime pour l'axe x (1er jour de chaque mois)
...
for i, y in enumerate(years[:]):  # Saute la premiÃ¨re annÃ©e si besoin
    ax = axs[i]
...
BV = watershed_root_legacy.Watershed(watershed_name=watershed_name, dem_path=None, out_path=out_path, load=True)
...
for irec, (name, drec) in enumerate(recharges):
    ...
    while (diff > ((gap/100) * half)):
        ...
        if not os.path.exists(os.path.join(BV.calibration_folder, model_name, '_matchingstreams','simflowf.shp')):
            valid_result = False
            break  # Sortie prÃ©maturÃ©e pour forcer retry ==> not really good
