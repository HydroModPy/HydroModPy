...
data_path = os.path.join(root_dir, "examples_legacy", "09S_short", "data")
...
specific_path = os.path.join(root_dir, "examples_legacy", "09S_short/")
dem_path = os.path.join(specific_path, "data", dem_filename)

# Create GeographicConfig with proper parameters
geo_config = GeographicConfig(
    catch_def='from_outlet_coord',
    dem_init_path=Path(dem_path),
    x_outlet=dem_coordinates[0],
    y_outlet=dem_coordinates[1],
    snap_dist=dem_coordinates[2],
    buff_area=dem_coordinates[3],
    polyg_shp_path=None,
...
BV = watershed_root_legacy.Watershed(load=False,
...
BV = watershed_root_legacy.Watershed(load=True,
                              initializing_object=initializing_object,
                              geographic_object=geographic_object,
                              save_object=False
                              )
...
list_folder = glob.glob(os.path.join(BV.calibration_folder, vers+'*'))

model_name = list_folder[0].split(os.path.sep)[-1]

pickle_file = os.path.join(list_folder[0], 'results_' + model_name + '.pkl')
with open(pickle_file, 'rb') as f:
    d = pickle.load(f)
...
shp_pathlines = gpd.read_file(os.path.join(calibration_folder, model_name, '_postprocess/_particles', 'pathlines_weighted.shp'))
shp_endpoints = gpd.read_file(os.path.join(calibration_folder, model_name, '_postprocess/_particles', 'starting_weighted.shp'))
...
for i in range(len(concobj_1c_fil_surf)):
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(figures_dir, vgif_name+'_'+str(i)+'_'+model_name+'.png'), dpi=300, bbox_inches='tight')
...
if plot_gif == True:
    ...
    if len(image_paths) > 0:
        ...
        ### Method 2
        images[0].save(os.path.join(figures_dir, '_' + gif_name), save_all=True, append_images=images[1:], optimize=True, duration=200, loop=0)
    else:
        print(f"âš  No PNG files found in {figures_dir} matching pattern {begin_by}*.png")
