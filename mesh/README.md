...
Ce package contient une brique autonome pour relire un bundle de
...
- `mesh` ne va pas importer directement le lecteur interne
  HydroModPy ;
...
- `mesh` execute bien le fichier `reader.py` present dans le
  bundle ;
...
- [models.py](/c:/codes/HydroModPy-GH/mesh/models.py)
  Definit les dataclasses, protocoles et constantes partages.
- [config.py](/c:/codes/HydroModPy-GH/mesh/config.py)
  Lit et valide le TOML.
- [toml_schema.py](/c:/codes/HydroModPy-GH/mesh/toml_schema.py)
  Porte le schema Pydantic du TOML et les descriptions courtes des parametres.
- [bundle_loading.py](/c:/codes/HydroModPy-GH/mesh/bundle_loading.py)
  Charge `reader.py` du bundle et reconstruit l'objet de travail.
- [summary.py](/c:/codes/HydroModPy-GH/mesh/summary.py)
  Construit le resume JSON compact.
- [visualization.py](/c:/codes/HydroModPy-GH/mesh/visualization.py)
  Construit les objets matplotlib et assemble la figure finale.
- [workflow.py](/c:/codes/HydroModPy-GH/mesh/workflow.py)
  Orchestre l'execution complete.
- [run_visualization.py](/c:/codes/HydroModPy-GH/mesh/run_visualization.py)
  Fournit le point d'entree en ligne de commande.
- [examples/config_example.toml](/c:/codes/HydroModPy-GH/mesh/examples/config_example.toml)
  Exemple generique de configuration a adapter a un bundle reel.
- [examples/config_mesh_catchment_outlet_5.toml](/c:/codes/HydroModPy-GH/mesh/examples/config_mesh_catchment_outlet_5.toml)
  Exemple pret a l'emploi pour un bundle concret.
- [environment.yml](/c:/codes/HydroModPy-GH/mesh/environment.yml)
...
mesh_distribution_package/
  mesh/
    ...
    workflow.py
    examples/
      config_mesh_catchment_outlet_5.toml
  sample_bundle/
    mesh_2d.msh
    nodes.csv
    cells.csv
...
`mesh`. Ceux-ci sont dans les CSV.
...
Le code de `mesh` peut le charger pour enrichir le resume final,
...
`mesh` charge dynamiquement ce fichier au lieu d'importer le
...
[load_visualization_data](/c:/codes/HydroModPy-GH/mesh/bundle_loading.py).

Cette fonction retourne un objet
[MeshVisualizationData](/c:/codes/HydroModPy-GH/mesh/models.py)
qui regroupe :
...
Depuis la racine du depot autonome :

```bash
conda env create -f mesh/environment.yml
...
Commande recommandee depuis la racine du depot autonome :

```bash
python -m mesh.run_visualization --config mesh/examples/config_mesh_catchment_outlet_5.toml
```
