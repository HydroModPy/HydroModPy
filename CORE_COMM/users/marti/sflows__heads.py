...
recharge = 1e-4  #m/j
defKR = np.logspace(-1.5,3,15)
deflaynb = [5]
defbc = [1000, 1500, 2000]
bottom=-1000
...
for lay_nb in deflaynb:
    for bc_left in defbc:
        for KR in defKR:   
            KR_name = round(KR,3)
            modeldir = 'D:/emarti/Tarapaca/out/Tarapaca/results_simulations/KR_' + str(KR_name) + '_layers_nb_' + str(lay_nb) + '_rch_' + str(recharge) + 'mj_bchead_'+str(bc_left)+'m_bottom_' + str(bottom) +'/'

            print('Performing simulation for model : KR = ' +str(KR_name)+' layer_nb='+str(lay_nb)+' bc='+str(bc_left))
            namepath      = 'KR_' + str(KR_name) + '_layers_nb_' + str(lay_nb) + '_rch_' + str(recharge) + 'mj_bchead_'+str(bc_left)+'m_bottom_' + str(bottom)
