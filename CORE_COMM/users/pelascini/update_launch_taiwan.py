...
if user == 'Ronan':
    ...
    # Path to the data folder
    data_path = "D:/Users/abherve/ONEDRIVE/OneDrive - Université de Rennes 1/PHD/4_model/TAIWAN/_data/"
    # Path where the results will be stored
    out_path = "D:/Users/abherve/TAIWAN/"
    # Figure folder outputs
    res_path = out_path + '_outputs/'
    # Modflow
...
watershed_names = ['Mini'] #['West1','West2','East1','East2','East3','East4','South1','South2','South3']
...
for watershed_name in watershed_names[:] :
    ...
    # Recharge
    init_rech = 1 / 365 / 24 # or 'mean' or 'first' or float
    
    #### FUNCTION NETCDF WITH REHCARGE ####
    
    recharge = pd.read_csv(stable_folder+'hydrology/mean_recharge_watershed.csv') # 15 days
    recharge.columns = ['values']
...
for watershed_name in watershed_names[:] :
    ...
    for simul in simul_list:
        ...
        Cmod.iloc[0] = init_rech

        fig, ax = plt.subplots(1,1, figsize=(5,3))

        axb = ax.twinx()
        axb.set_ylabel('R [mm/h]', rotation=270, labelpad=25)
        axb.bar(Cmod.index, Cmod,
                color='blue', edgecolor='blue', lw=2.5, alpha=0.1)
        axb.invert_yaxis()
        # axb.set_yscale('log')
    
        ax.set_ylabel(scan.upper() + ' [?]')
        ax.plot(Qmod, color='red', lw=2, label='modeled')
        # ax.set_yscale('log')

#%% PLOT FIGURES SEEPAGE

npy = np.load("D:/Users/abherve/TAIWAN/Mini/results_simulations/T-Mini-1_1_15.0-0.2-30/_watershed/seepage_areas.npy", allow_pickle=True).item()
for i in range(len(npy)):
    print(str(i)+'/'+str(len(npy)))
    fig, ax = plt.subplots(1,1, figsize=(5,3))
    targ = npy[i]
    targ[targ<=0] = np.nan
    ax.imshow(targ)
    fig.savefig('D:/Users/abherve/TAIWAN/Mini/results_simulations/T-Mini-1_1_15.0-0.2-30/_figures/png/'+
            str(i)+'.png')
    plt.close()
