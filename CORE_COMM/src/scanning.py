...
class run_calibration:
    def __init__(self, krval=500, compt=0, df=pd.DataFrame(),
        ...
        self.df.loc[self.compt,'Ratio'] = round(self.store.sim_to_obs_mean / self.store.obs_to_sim_mean, 2)
...
class scanning_loop:
    def __init__(self, first=1, last=10000, gap=10,
        ...
        self.sim_list = glob(self.out_path+self.watershed+'\\'+'scan*')
        ...
        self.krval = np.geomspace(self.first, self.last, self.gap)
        
        self.compt=0
        for val in self.krval:
            self.calibration = run_calibration(krval=val, compt=self.compt, df=self.df,
                                             watershed=self.watershed,
        ...
        self.save_name = self.watershed+'\\'+self.watershed+'_scanning.csv'
