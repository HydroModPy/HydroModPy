...
class modpath_model:
	...
	def build_modpath_model(self, verbose=True):
		...
		self.mf = flopy.modflow.Modflow.load(self.model_name+'.nam',model_ws=self.full_path, verbose=False, check=False)
		dis = flopy.modflow.ModflowDis.load(dis_file, self.mf)
		...
		sim = flopy.modpath.Modpath6Sim(model=self.mp, option_flags=[2, 1, 1, 1, 1, 2, 2, 1, 1, 2, 1, 1],
		...
		mpbas = flopy.modpath.Modpath6Bas(self.mp, hnoflo=-9999.0, hdry=-100, def_face_ct=0, laytyp=laytype, ibound=iboundData,
