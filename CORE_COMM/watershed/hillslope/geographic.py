...
class Geographic:
    ...
    def __init__(self, dem_path, x, y, snap_dist=150, buff_dist=1000, crit_drainage=400,
                 out_path=os.path.dirname(os.path.dirname(__file__))+'\\output\\'):
        print('Extraction des données géographiques')
        
        self.processing(dem_path, x, y, snap_dist, buff_dist, crit_drainage, out_path)
        self.post_processing_dem()

    def processing(self, dem_path, x, y, snap_dist, buff_dist, crit_drainage, out_path):
        ...
        # Correction
        self.fill = gis_path + 'region_fill.tif'
        wbt.fill_depressions(dem_path, self.fill) # or # wbt.breach_depressions(dem_path, fill, 2, 75*8)
        # Flow direction
        self.direc = gis_path + 'region_direc.tif'
        wbt.d8_pointer(self.fill, self.direc, esri_pntr=False)
        # Flow accumulation
        acc = gis_path + 'region_acc.tif'
        wbt.d8_flow_accumulation(self.fill, acc)#, log=True)
        # Extract streams
        self.streams = gis_path + 'region_streams.tif'
        wbt.extract_streams(acc, self.streams, threshold=crit_drainage)
        ...
        wbt.watershed(self.direc, outlet_snap_shp, watershed, esri_pntr=False)
        ...
        wbt.clip_raster_to_polygon(self.fill, buffer, watershed_buff_fill)
        ...
        wbt.clip_raster_to_polygon(self.direc, buffer, watershed_buff_direc)
        ...
        wbt.clip_raster_to_polygon(self.fill, self.watershed_shp, self.watershed_fill, maintain_dimensions=True)
        ...
        # Clip stream network from watershed shapefile polygon
        self.watershed_stream = gis_path + 'watershed_stream.tif'
        wbt.clip_raster_to_polygon(self.streams, self.watershed_shp, self.watershed_stream, maintain_dimensions=True)
        ...
        wbt.clip_raster_to_polygon(self.fill, box_buffer, watershed_box_buff_fill)
        ...
        wbt.clip_raster_to_polygon(self.direc, box_buffer, watershed_box_buff_direc)
