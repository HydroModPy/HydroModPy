...
TO DO : tests to compare values in the ref input file and the generated alternative input file
...
def get_indexes_to_remain_approx3(number_lines, period):
    ligne = 0
    indexes = [0] # Keeping the first value = initialisation
    taille_period = len(period)
    i = 0
    while (ligne < number_lines-1):
        ligne += period[i]
        ...
        if (i == (taille_period-1)):
            ...
            i += 1
    #print("indexes to remain :", indexes)
    return indexes
...
def aggregate_values(df, indexes, nb_rows, approx):
    '''
        TODO
    '''
...
def no_aggregate_values(df, indexes, nb_rows):
    '''
        TODO
    '''
    df.loc[0,'rech'] = float(df['rech'].mean()) #df['rech'][0]
...
def manipulate_ref_input_file(template_name,  approx, rate, steady, time_step=None): #, periodValue
    ...
    if steady :
        ...
        df = aggregate_values(df, indexes, len(df.index), approx)
...
def manipulate_ref_input_file_no_agg(template_name,  approx, rate, steady, time_step=None): #, periodValue
    df = extract_df_from_ref_input_file(template_name)
    if steady :
        df.loc[0,'rech'] = float(df['rech'].mean()) #df['rech'][0]
        print("mean init for inputfile: ", df.loc[0,'rech'])
        ...
        df = no_aggregate_values(df, indexes, len(df.index))
        df = keep_only_rows_with_indexes(df, indexes)
...
def generate_custom_input_file_no_agg(model_name, input_name, approx, rate, chronicle, steady, time_step=None):
    folder_path = '/'.join((os.path.dirname((os.path.abspath(__file__)))).split('/')[:-1])
    chronicle_file = pd.read_table(folder_path + "/data/chronicles.txt", sep=',', header=0, index_col=0)
    template_file = chronicle_file.template[chronicle]
    df = manipulate_ref_input_file_no_agg(template_file,  approx, rate, steady, time_step=time_step)
    if model_name is None:
        model_name = hlp.generate_model_name(chronicle, approx, rate, ref=False)
    output_name = write_custom_input_file(model_name + "_no_agg", df)
    return output_name
