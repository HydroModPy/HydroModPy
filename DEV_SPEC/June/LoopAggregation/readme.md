...
```bash
python3 settings_model.py -site $SITE -approx $APPROX -chr $CHR -rate $RATE ${REF}
```
...
##### Generate the input file for the corresponding Aggregation rate
```python
from InputFileManipulation import generate_custom_input_file

InputFileManipulation.generate_custom_input_file(model_name, input_name, approx, rate, chronicle, steady)
```
