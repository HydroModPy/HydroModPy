...
.. note::
   ...
   in ``hydromodpy.simulation.execution.trial`` and the CLI in
   ``hydromodpy.calibration.runner``.
...
- ``hydromodpy.calibration`` - the calibration package (engine,
  parameters, objective, optimizer registry, diagnostics, cases).
- ``hydromodpy.simulation.execution.trial`` - the prepare-once /
