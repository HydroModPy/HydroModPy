...
`testbed` vs `simulation` vs `comparison`
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

``testbed`` is the reproducible evidence layer for method robustness. It is
not a solver, and it is not a second simulation planner.

The contract is:

- ``workflow``: user-facing operation selected by ``workflow = "..."``;
- ``subject``: method domain under test, such as ``mesh`` or ``flow``;
- ``runner``: child workflow used for one variant, such as ``mesh_catchment``
  or ``simulation``;
- ``variant``: one concrete child configuration generated by overlaying an
  axis-specific payload on a base config;
- ``axis``: dimension varied by the testbed, such as mesh resolution,
  hydraulic conductivity, solver tolerance, or boundary-condition choice;
- ``metric``: scalar evidence extracted from the child summary or from the
  persisted run catalog.

This boundary keeps the roles separate:

- ``simulation`` owns one physical execution and persists one ``Run``;
- ``comparison`` compares completed or generated simulation variants against a
  reference case;
- ``testbed`` owns the experimental matrix and evidence collection, whether
  the child runner builds a mesh or runs a flow simulation.

For flow testbeds, the generated children are still ordinary
``workflow = "simulation"`` TOMLs. After execution, the testbed may reopen the
``SimulationCatalog`` to collect duration, solver status, parameters, budget
summaries, mass-balance diagnostics, and field summaries. That catalog
inspection enriches the evidence layer without moving flow execution logic out
