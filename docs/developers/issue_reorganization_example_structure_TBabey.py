...
if case == "bw_seepage":
    ...
    tif_seep = (
        BV.simulations_folder + "/" + model_name + "/_postprocess/_rasters/seepage_areas_t(0).tif"
    )
    tif_seep_clip = (
        BV.simulations_folder
        + "/"
        + model_name
        + "/_postprocess/_rasters/seepage_areas_t(0)_clip.tif"
    )
...
if case == "bw_wells":
    ...
    bore = bore * 0
    xbore, ybore = BV.geographic.crs_to_iloc(x_crs=214594.2, y_crs=6758954.1, crs_proj="EPSG:2154")
    bore[ybore, xbore] = 1
    # bore[20,20] = 1
    # bore[40,48] = 1
    # bore[38,22] = 1
    # bore[28,21] = 1
    particles_folder = os.path.join(
        BV.simulations_folder + "/" + model_name, "_postprocess", "_particles"
    )
    toolbox.create_folder(particles_folder)
    toolbox.export_tif(
        BV.geographic.watershed_box_buff_dem,
        bore,
        BV.geographic.simulations_folder
        + "/"
        + model_name
        + "/"
        + "_postprocess/_particles/"
        + "synthetic_boreholes.tif",
        0,
    )
    tif_bore = (
        BV.geographic.simulations_folder
...
if case == "fw_recharge":
    ...
    tif_recharge_clip = (
        BV.simulations_folder + "/" + model_name + "/_postprocess/_rasters/recharge_area.tif"
    )
    wbt.clip_raster_to_polygon(
        tif_recharge,
        BV.stable_folder + "/geographic/watershed.shp",
...
outflow = imageio.imread(
    simulations_folder + model_name + "/_postprocess/_rasters/accumulation_flux_t(0).tif"
)
...
shp_pathlines = gpd.read_file(
    simulations_folder + model_name + "/_postprocess/_particles/pathlines_weighted.shp"
)
...
shp_endpoints = gpd.read_file(
    simulations_folder + model_name + "/_postprocess/_particles/starting_weighted.shp"
)
...
rasterio.plot.show(
    dem_data, ax=ax, transform=dem_rio.transform, cmap="Greys", alpha=0.7, zorder=-10
)
