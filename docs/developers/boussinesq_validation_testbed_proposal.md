...
The reusable campaign-level post-processor is implemented here:
...
The individual method-comparison HTML renderer is implemented separately in the
generic comparison package:

```text
hydromodpy/analysis/comparison/web/
hydromodpy/analysis/comparison/web_report.py
hydromodpy/analysis/comparison/visuals.py
```

This code is not specific to the Boussinesq/MODFLOW 6 synthetic campaign. The
specific comparison is selected by the comparison TOML and by the generated
`comparisons/<comparison_id>/comparison_manifest.json`; the HTML renderer only
reads a `comparison_root` and renders the standard comparison artifacts found
there. New campaigns should therefore add or reference comparison configs and
manifests rather than create campaign-specific HTML rendering code.
...
For comparison-index-only campaigns, the synthesis can intentionally omit
`web_synthesis/cases/<case_id>.html` and link directly to:

```text
<output_root>/comparisons/<comparison_id>/web/index.html
```

Generated HTML, PNG, CSV, JSON, catalogs, workspaces and `.zarr` stores are local
artifacts and should stay out of version control. Version the TOML inputs, runner
scripts, report-generation code, tests and documentation instead.
...
closer to the natural cases targeted later. For the Boussinesq candidate, the
current rule is case 1 only: no explicit Cauchy/Robin drainage term. Existing
drainage entries are neutralized in the Boussinesq overlays with
`flow.bc.cauchy.drainage.value = "0.0 m2/s"`, so the VI upper obstacle remains
the aquifer top (`h <= z_top`). The initial head is generated by the generic
steady-state initial-condition workflow: each solver first runs a permanent
calculation with the mean recharge, then uses the resulting head field as the
...
With this zero-conductance Boussinesq setup, the PETSc upper VI bound is not
relaxed. The explicit drainage law `C * max(h - z_top, 0)` is reserved for a
future, separate case-2 experiment; it is not part of the current Boussinesq
validation campaign.
...
The generic comparison standard should go slightly further than simply adding
`is_initial_state`. The temporal selector must be explicit about the physical
object being compared:

- `time = "initial_state"` targets the initial condition itself and requires an
  exported initial-state slice;
- `time = "first_computed"` targets the first transient result after the first
  completed period;
- `time = "all"` may include an initial-state row when a solver exports it, but
  metrics must only compare rows with compatible `time_role` values;
- `time = "first"` remains a legacy "first available row" selector and should
  not be used in validation cases where solvers may differ in initial-state
  export policy.

The comparison metrics therefore use `time_role` as part of the physical
contract: `initial_state`, `state_snapshot`, and `period_value` are not
interchangeable fallback matches.
In the current synthetic campaign, the shared map formerly named
`head_map_initial` is replaced in the configuration by
`head_map_first_computed`, because MODFLOW 6 starts its exported head series at
the first computed transient step whereas Boussinesq also exposes the explicit
initial state. A true initial-condition comparison should be reintroduced only
when both methods export a comparable `initial_state` row.
...
| --- | --- | --- | --- | --- |
| MODFLOW 6 | `RCHA` -> `recharge_total_m3_s` | `DRN` -> `drainage_total_m3_s`; `CHD` only when a prescribed-head boundary is active | `STO-SS`/`STO-SY` -> `storage_change_total_m3_s` | `drainage_total_m3_s` when the MODFLOW reference carries a DRN package |
| Boussinesq | `budget/recharge` -> `recharge_total_m3_s`; signed `budget/well` if active | `budget/drain` should stay inactive for the zero-conductance case; `budget/surface_excess` -> `surface_excess_total_m3_s` | reconstructed from `head`, `z_bottom`, `z_top`, cell area and `storage_coefficient` | `surface_excess_total_m3_s`; balance-implied outflow remains a diagnostic when a method lacks a native outflow term |
...
head_map_wet_year1__fine_raster_map_comparison.png
  -> the only head-field figure shown by default: MF6 and Boussinesq after the
     first wet season, interpolated on the same grid
...
The compact synthetic pages intentionally suppress budget diagnostic figures,
aggregated outflow/flux chronicles, non-interpolated head maps, head-field
triptychs/difference maps, runtime bars, and the grouped dashboard with the five
head chronicles. The head chronicles are kept as separate comparison figures so
each observation point can be read independently.
...
The former low/high drainage-conductance synthetic cases are kept only as
zero-conductance placeholders and are no longer part of the default campaign.
Boussinesq validation stays on case 1: strict free-surface obstacle, no explicit
Cauchy drainage.
