...
:doc:`design_patterns <design_patterns>`, :doc:`CLI <cli>`.
...
.. list-table::
   ...
   * - Fichier
     - Rôle
   * - ``config.json``
     - JSON Schema complet de ``HydroModPyConfig``
   * - ``config_meta.json``
     - Sections TOML ordonnées, groupes UI, titres
   * - ``field_validators.json``
     - Mapping plat ``field_path -> validator_type``
