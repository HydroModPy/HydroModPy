...
- input-data cache shared by several projects on the same geographic
  area;
- project TOMLs (``project.toml``) and overlays;
...
.. code-block:: text
   ...
   └── projects/
       ├── my_basin/
       │   ├── project.toml       project config (Pydantic root)
...
Given a project ``project.toml``, HydroModPy resolves the
...
2. **Scaffold** discovery: the TOML lives at
   ``<workspace>/projects/<name>/project.toml`` and ``<workspace>``
