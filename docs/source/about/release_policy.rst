...
This page defines how HydroModPy is versioned, tagged, and published. It
is the contract between maintainers and users about what a version number
means and how a release reaches PyPI.

Semantic versioning
-------------------

HydroModPy follows `Semantic Versioning 2.0.0 <https://semver.org/>`_.
...
Pre-releases use ``X.Y.ZrcN`` (for example ``1.0.0rc1``).
...
3. Be tracked in ``docs/_internal/notes/dev/legacy_remaining.md`` until
   removal.
...
- Tags are immutable. A botched release is fixed by tagging a new PATCH,
...
3. The ``publish.yml`` workflow takes over:
   ...
   - Uploads to PyPI via ``pypa/gh-action-pypi-publish`` using Trusted
     Publishers (OIDC). No API tokens are stored as secrets.
...
- :doc:`roadmap` for upcoming themes.
- ``SECURITY.md`` (repository root) for the supported-versions policy
