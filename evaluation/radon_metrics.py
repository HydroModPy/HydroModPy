...
def compute_radon_metrics(root: Path) -> dict[str, Any]:
    ...
    for path in iter_python_files(root):
        try:
            ...
            metrics.append({"path": str(path), "error": str(exc)})
        ...
        metrics.append(
            {
                ...
                "complexity": safe_complexity(text),
                "maintainability_index": safe_maintainability(text),
                "halstead": safe_halstead(text),
            }
        )
