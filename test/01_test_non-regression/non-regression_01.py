...
ax = axs[0]
axb = axs[0].twinx()
ax.step(ref_timeseries.index, ref_timeseries['recharge']*30*1000, lw=8, color='navy', label='Recharge total', where='pre', clip_on=False)
ax.step(ref_timeseries.index, ref_timeseries['outflow_drain']*30*1000, lw=5, color='dodgerblue', alpha=1, label='Outflow at the outlet', where='pre', clip_on=False)
...
ax.legend(prop={'size': 12})
axb.bar(ref_timeseries.index, well_all_fluxes_plot, clip_on=False, width=5, lw=0, color='purple', label='SIMULATED: wells')
# axb.set_ylim(-110,0)
...
ax = axs[1]
axb = axs[1].twinx()
ax.step(sim_timeseries.index, sim_timeseries['recharge']*30*1000, lw=8, color='navy', label='Recharge total', where='pre', clip_on=False)
ax.step(sim_timeseries.index, sim_timeseries['outflow_drain']*30*1000, lw=5, color='dodgerblue', alpha=1, label='Outflow at the outlet', where='pre', clip_on=False)
ax.set_xlim(pd.to_datetime('2017-01'), pd.to_datetime('2018-01'))
# ax.set_ylabel('Output flow results [mm/month]')
ax.set_ylim(0, 110)
ax.xaxis.set_major_locator(mdates.YearLocator())
...
ax.xaxis.set_minor_formatter(mdates.DateFormatter('%m'))
ax.legend(prop={'size': 12})
axb.bar(sim_timeseries.index, well_all_fluxes_plot, clip_on=False, width=5, lw=0, color='purple', label='SIMULATED: wells')
# axb.set_ylim(-110,0)
axb.set_ylabel('Sum of pumping in wells [L$^3$/T]', rotation=270, labelpad=25)
ax.set_title('SIMULATED: time 1/12')
axb.legend(prop={'size': 12}, loc='lower left')
