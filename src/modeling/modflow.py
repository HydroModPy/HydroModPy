...
class Modflow:
    ...
    def pre_processing(self):
        ...
        self.nwt = flopy.modflow.ModflowNwt(self.mf, 
                                            ...
                                            maxiterout=5000,
                                            thickfact=self.thickfact, # default 1e-05
                                            linmeth=1,
                                            iprnwt=1,
                                            ibotav=1,
                                            options='COMPLEX',
        ...
        if self.use_lakeres:
            stages, lakarr_lay0, laklay_top, bdlknc_lay0, flux_data, self.dem = self.lakeres.format_to_modflow(
                self.geographic, self.recharge, self.nper, self.thickfact, self.dem, self.dem_watershed_path)
...
#             self.geographic, self.recharge, self.nper, self.thickfact)
# =============================================================================
...
# =============================================================================
        ...
        def check_water_flow_connectivity(grid):
            ...
            for z in range(layers - 1):  # Focus on flow between layers
                logging.debug(f"Checking layer {z}")
                for y in range(rows):
                    for x in range(cols):
