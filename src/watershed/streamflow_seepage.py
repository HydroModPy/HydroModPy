...
class streamflow_seepage:
    ...
    def __init__(self, area='watershed', icalc:int=0, thickm:float=0.1, 
        ...
        ----------
        area : str, optional
            'river' | 'watershed' (default) | 'domain'
        ...
        self.sfr_seepage_folder = os.path.join(self.geographic.stable_folder, "sfr_seepage")
        ...
        if rchlen is None: # average channel lenght in each cell
            self.rchlen = self.geographic.resolution * (1/4 + 1/(2*np.sqrt(2))) # average straight euclidien length
        else:
            self.rchlen = rchlen
            
        self.area = area
        
        self.critical_mode = None


    #%% Define seepage area where StreamFLow Routing will be applied
    def SFR_seepage_area(self, dem):
        self.dem = dem
        # ---- Define the area of SFR seepage
        self.direc, _, _, _ = toolbox.load_to_numpy(self.geographic.watershed_box_buff_direc)
...
# =============================================================================
    ...
    def load_reach_data(self, csv_path):
# =============================================================================
#         # version recarray
#         self.reach_data = np.genfromtxt(reach_data_path, delimiter=';', names=True)
# =============================================================================
        self.reach_data = pd.read_csv(csv_path, sep=';')
        return self.reach_data
    
    def load_segment_data(self, csv_path):
# =============================================================================
#         # version reacarray
#         self.segment_data_1 = np.genfromtxt(segment_data_path, delimiter=';', names=True)
# =============================================================================
        self.segment_data_1 = pd.read_csv(csv_path, sep=';')   
        return self.segment_data_1
    
    
    #%% AUTOMATICALLY COMPUTE REACH AND SEGMENT DATA
    def compute_data(self):
        ...
        self.reach_data = pd.DataFrame(
            index = range(0, self.direc[self.direc == 1].sum()), 
    ...
    #%% UPDATE INFO FOR CRITICAL AREA COMPUTATION
    def correct_critical_cells(self, hcond:float=0.012, area:str='sinks', 
    ...
    def critical_area(self):
        ...
        if self.critical_mode == "sinks":
            crit_area = self.geographic.depressions_data
            ...
            for dep_val in np.unique(crit_area):
                crit_area[crit_area == dep_val] = acc_map[crit_area == dep_val].sum()
            ...
            for nseg, s in self.segment_data_1.iterrows():
                ...
                if len(r) == 1:
                    ...
                    if self.sink[r['i'], r['j']] >= self.sink_threshold:
