...
class Watershed :
    ...
    def add_hydrodynamic(self):
        self.hydrodynamic = hydrodynamic.Hydrodynamic(nrow=self.geographic.y_pixel, ncol=self.geographic.x_pixel, box_dem=self.geographic.watershed_box_buff_dem)
        self.elt_def.append('hydrodynamic')
    ...
    def add_hydrology(self, hydrology_path, types_obs = ['streams'], fields_obs = ['FID'], reset = False):
        ...
        self.hydrology = hydrology.Hydrology(out_path=self.watershed_folder,
                                             types_obs=self.types_obs, fields_obs=self.fields_obs,
                                             geographic=self.geographic, hydro_path=self.hydrology_path)
        self.elt_def.append('hydrology')
        self.save_object()
    ...
    def run_modflow(self, ident: str = 'modflow',run: bool = True, modpath_sim: bool = False, 
                    ...
                    verti_k_tconst: list = None,
                    verti_k_thetero: list = None,
                    from_3D=False,
                    xyz_3D=None,
                    val_3D=None,
                    idx_3D=None,
