...
from watershed import geometric
...
class Modflow():
    ...
    def __init__(self, geographic, sink_fill = False, box=True,
        ...
        self.verti_k_tconst = verti_k_tconst
        self.verti_k_thetero = verti_k_thetero
    ...
    def pre_processing(self, verbose=False):
        ...
        if self.from_3D == True:
            z_discretization = self.mf.modelgrid.zvertices
            hk_3D, sy_3D = geometric.geol(self.xyz_3D,
                                          self.val_3D,
                                          self.idx_3D,
                                          self.dem_path,
                                          z_discretization,
                                          extrapol_method=self.ext_3D,
                                          load_index=False)
        ...
        if self.verti_k_tconst != None:
            for j in range(len(self.verti_k_tconst)):
                # print('j', j)
                for i in range(len(self.zbot)):
                    # print('i', i)
                    k_val = self.verti_k_tconst[j][0]
                    d1 = self.verti_k_tconst[j][1][0]
                    d2 = self.verti_k_tconst[j][1][1]
                    cond_d1 = (self.dem - d1)
                    cond_d2 = (self.dem - d2)
        ...
        if self.verti_k_thetero != None:
            print('   Verti K heterogenous')
            for j in range(len(self.verti_k_thetero)):
                k_val = self.verti_k_thetero[j][0]
                d1_array = self.verti_k_thetero[j][1][0]
                d2_array = self.verti_k_thetero[j][1][1]
                cp = 0
                # for r in range(d1_array.shape[0]):
                #     for c in range(d2_array.shape[1]):
                d1 = d1_array#[r, c]
                d2 = d2_array#[r, c]
                cond_d1 = (self.dem - d1)
                cond_d2 = (self.dem - d2)
                ...
                for i in range(len(self.zbot)):
                    ...
                    self.hk[i][mask] = k_val
                # if not cp % 100:
                print('   '+str(cp)+'/'+str(d1_array.shape[0]*d2_array.shape[1]))
                cp+=1
            # print(k_val)
