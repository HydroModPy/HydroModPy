"""Validate the steady linearized unconfined hillslope-drainage case."""
...
def test_linearized_unconfined_hillslope_drainage_1d_matches_reference_profile(
    solver: str,
    require_modflow: bool,
    require_modflow6: bool,
) -> None:
    """Run the case and compare the mean head profile to the analytical reference."""
    ...
    comparison = run_linearized_unconfined_hillslope_drainage_comparison(
    ...
    assert comparison.min_numerical_clearance_m > 0.0, (
        "The numerical profile falls below topography, which violates the "
        "distributed-drainage analytical assumptions."
    )
