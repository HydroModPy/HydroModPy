...
| 47 | SFR-fed reservoir E2E (synthetic Cheze) | min stage / band coverage / RMSE / flat-golden max-abs / control drawdown / budget | min `> 92.0 m` (floor = abacus bottom + 1 m); `>= 90 %` of steps in `[golden_min - 0.5, golden_max + 0.5]`; RMSE vs golden `< 1.0 m`; golden variance `< 1e-4 m2` switches NSE `> 0.90` to max-abs `< 0.10 m`; control final stage `< sim - 0.1 m` and monotone declining; listing discrepancy `<= 1 %` | Golden chronicle pinned from the first converged `hmp run` of the committed valley fixture (`tests/data/sfr_cheze/golden_stage.csv`, `tests/e2e/test_sfr_cheze_e2e.py`) | The compressed Cheze acceptance: the spillway-held level is flat to the mm, so NSE vs the golden degenerates and the stricter 10 cm max-abs replaces it. The 0.5 m band and 1 m RMSE mirror the real-Cheze observed-band gates; the negative control (same model minus `sfr`) reproduces the draining regime the SFR feed fixes |
