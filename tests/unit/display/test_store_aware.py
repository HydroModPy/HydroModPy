...
class TestLoadFlowTimeseriesFromStore:
    def test_returns_dataframe_with_stored_variables(self, store):
        sid = str(uuid4())
    ...
    def test_returns_none_when_no_data(self, store):
        sid = str(uuid4())
        store.register_simulation(sid, solver="modflownwt")
        assert _load_flow_timeseries_from_store(store, sid) is None

    def test_partial_variables(self, store):
        sid = str(uuid4())
        store.register_simulation(sid, solver="modflownwt")

        idx = pd.date_range("2020-01-01", periods=3, freq="ME")
        store.write_timeseries(sid, _CATCHMENT_STATION, "watertable_depth", pd.Series([4.1, 4.2, 4.3], index=idx))
    ...
    def test_all_flow_variables(self, store):
        sid = str(uuid4())
        store.register_simulation(sid, solver="modflownwt")

        idx = pd.date_range("2020-01-01", periods=4, freq="ME")
        for var in _FLOW_TIMESERIES_VARIABLES:
            store.write_timeseries(sid, _CATCHMENT_STATION, var, pd.Series(np.ones(4), index=idx))
...
class TestLoadFieldDictFromStore:
    def test_loads_field_as_dict(self, store):
        sid = str(uuid4())
        store.register_simulation(
            sid, solver="modflownwt", n_cells=10, n_layers=1, n_timesteps=3,
        )

        for t in range(3):
            values = np.random.rand(1, 10)
            store.write_field(sid, "head", t, values, n_timesteps=3)

        result = _load_field_dict_from_store(store, sid, "head")
        assert result is not None
        assert len(result) == 3
        assert all(isinstance(v, np.ndarray) for v in result.values())

    def test_returns_none_for_missing_variable(self, store):
        sid = str(uuid4())
        store.register_simulation(sid, solver="modflownwt", n_cells=10, n_layers=1, n_timesteps=1)
        result = _load_field_dict_from_store(store, sid, "nonexistent")
        assert result is None

    def test_returns_none_for_missing_sim(self, store):
        result = _load_field_dict_from_store(store, "no_such_sim", "head")
        assert result is None
...
class TestPlotFlowSuiteWithStore:
    ...
    def test_uses_store_when_available(self, monkeypatch, store):
        ...
        for var in ["recharge", "outflow_drain", "runoff", "watertable_depth"]:
            store.write_timeseries(sid, _CATCHMENT_STATION, var, pd.Series(np.ones(5), index=idx))
        ...
        plot_flow_suite(result, options, store=store, sim_id=sid)
        assert not csv_called, "should NOT have called CSV loader when store has data"


class TestPosthocContextFromResultStore:
    def test_from_result_store_discovers_runs(self, store, tmp_path):
        ...
        # Register simulations in store
        sid_a, sid_b = str(uuid4()), str(uuid4())
        store.register_simulation(sid_a, solver="modflownwt", name="run_a")
        store.finalize(sid_a)
        store.register_simulation(sid_b, solver="modflownwt", name="run_b")
        store.finalize(sid_b)

        ctx = PosthocContext.from_result_store(project, store)
        assert len(ctx.runs) == 2
        run_ids = {r.run_id for r in ctx.runs}
        assert "run_a" in run_ids
        assert "run_b" in run_ids

    def test_from_result_store_falls_back_to_filesystem(self, store, tmp_path):
        from hydromodpy.analysis.display.posthoc import PosthocContext
