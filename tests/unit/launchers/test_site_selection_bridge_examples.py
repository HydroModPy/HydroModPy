...
EXAMPLE_ROOT = (
    Path(__file__).resolve().parents[3] / "examples" / "projects" / "18_site_selection_to_testbed"
)
...
def test_site_selection_manifest_testbed_example_expands_catalog() -> None:
    ...
    assert (
        cfg.catalog.source_manifest_path
        == (EXAMPLE_ROOT / "fixtures" / "site_selection_manifest.json").resolve()
    )
