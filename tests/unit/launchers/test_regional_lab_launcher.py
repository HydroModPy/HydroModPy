...
def test_regional_lab_compatibility_toml_plan_contract_is_stable(tmp_path: Path) -> None:
    ...
    assert [
        ...
        ("sim_reference::headwater_100km2_outlet_2", "simulation", "run_headwater_100km2_outlet_2.toml"),
        ("sim_reference::headwater_100km2_outlet_3", "simulation", "run_headwater_100km2_outlet_3.toml"),
        ("backend_compare::headwater_100km2_outlet_2", "comparison", "compare_headwater_100km2_outlet_2.toml"),
    ]
    assert [
...
def test_regional_lab_plan_projects_to_testbed_cases(tmp_path: Path) -> None:
    ...
    assert testbed_cases[0].case.variant.label == (
        "sim_reference / headwater_100km2_outlet_2"
    )
    assert testbed_cases[0].to_mapping() == {
        ...
        "status": "planned",
        "config_path": str(
            (tmp_path / "configs" / "run_headwater_100km2_outlet_2.toml").resolve()
        ),
        "runner": "simulation",
