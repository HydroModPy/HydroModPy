...
def test_ensure_for_process_rejects_unknown_process_type() -> None:
    factory = ProcessContextFactory()
    state = _build_state()

    with pytest.raises(ValueError, match="Unsupported process type 'quality'"):
        factory.ensure_for_process(state, "quality")
