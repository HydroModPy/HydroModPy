"""End-to-end regression test for examples/04.../example_04.py."""
...
GOLDEN_REFERENCE_FILE = (
    ...
    / "reference"
    / "golden_references"
    / "example_04_npy_signatures.json"
)
...
@pytest.mark.slow
def test_example_04_regression_on_npy_outputs(tmp_path, update_goldens):
    """Run example_04, then compare (or refresh) its golden signatures."""
    assert_required_executables()

    out_path = tmp_path / "example_04_outputs"
    run_legacy_example_script(
        script_path=EXAMPLE_04_SCRIPT,
        out_path=out_path,
