"""End-to-end regression test for examples/02.../example_02.py."""
...
GOLDEN_REFERENCE_FILE = (
    ...
    / "reference"
    / "golden_references"
    / "example_02_npy_signatures.json"
)
...
@pytest.mark.slow
def test_example_02_regression_on_npy_outputs(tmp_path, update_goldens):
    """Run example_02, then compare (or refresh) its golden signatures."""
    assert_required_executables()

    out_path = tmp_path / "example_02_outputs"
    run_legacy_example_script(
        script_path=EXAMPLE_02_SCRIPT,
        out_path=out_path,
