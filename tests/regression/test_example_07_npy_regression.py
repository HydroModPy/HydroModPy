...
GOLDEN_REFERENCE_FILE = (
    ...
    / "reference"
    / "golden_references"
    / "example_07_binary_signatures.json"
)
...
@pytest.mark.slow
def test_example_07_regression_on_binary_outputs(tmp_path, update_goldens):
    """Run example_07, then compare (or refresh) binary signatures."""
    assert_required_executables()

    out_path = tmp_path / "example_07_outputs"
    run_legacy_example_script(
        script_path=EXAMPLE_07_SCRIPT,
        out_path=out_path,
    ...
    model_ws = out_path / "Example_07_Hillslope" / "results_simulations" / model_name
